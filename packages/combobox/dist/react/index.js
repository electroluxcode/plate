...
var useComboboxInput = ({
  ...
  return {
    ...
    props: {
      ...
      onKeyDown: (event) => {
        if (cancelInputOnEscape && (0, import_platejs.isHotkey)("escape", event)) {
          cancelInput("escape", true);
        }
        if (cancelInputOnBackspace && cursorAtStart && (0, import_platejs.isHotkey)("backspace", event)) {
          cancelInput("backspace", true);
        }
        if (cancelInputOnArrowLeftRight && cursorAtStart && (0, import_platejs.isHotkey)("arrowleft", event)) {
          cancelInput("arrowLeft", true);
        }
        if (cancelInputOnArrowLeftRight && cursorAtEnd && (0, import_platejs.isHotkey)("arrowright", event)) {
          cancelInput("arrowRight", true);
        }
        const isUndo = import_platejs.Hotkeys.isUndo(event) && editor.history.undos.length > 0;
        const isRedo = import_platejs.Hotkeys.isRedo(event) && editor.history.redos.length > 0;
