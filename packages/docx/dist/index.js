...
var cleanDocxBrComments = (rootNode) => {
  (0, import_platejs.traverseHtmlElements)(rootNode, (element) => {
    if (element.tagName !== "BR") {
      return true;
    }
    if (element.nextSibling && (0, import_platejs.isHtmlComment)(element.nextSibling) && element.nextSibling.data === "[if !supportLineBreakNewLine]") {
      (0, import_platejs.removeHtmlNodesBetweenComments)(
        element.nextSibling,
...
var isHtmlOpEmpty = (element) => element.nodeName === "O:P" && element.textContent === import_platejs2.NO_BREAK_SPACE;
...
var cleanDocxEmptyParagraphs = (rootNode) => {
  (0, import_platejs2.traverseHtmlElements)(rootNode, (element) => {
...
var cleanDocxFootnotes = (rootNode) => {
  (0, import_platejs3.traverseHtmlElements)(rootNode, (element) => {
...
var getRtfImageHex = (imageData) => {
  ...
  if (bracketSplit.length > 2 && beforeBracket.includes("blipuid")) {
    return afterBracket.split(import_platejs4.SPACE).join("");
  }
  const spaceSplit = beforeBracket.split(import_platejs4.SPACE);
...
var END_OF_ID_SEQUENCE_CHARACTERS = [
  "\\",
  "{",
  import_platejs5.CARRIAGE_RETURN,
  import_platejs5.LINE_FEED,
  import_platejs5.SPACE
];
...
var getVShapes = (document2) => {
  const comments = (0, import_platejs6.getHtmlComments)(document2);
...
var cleanDocxImageElements = (document2, rtf, rootNode) => {
  ...
  (0, import_platejs7.traverseHtmlElements)(rootNode, (element) => {
    ...
    const dataUri = `data:${rtfImage.mimeType};base64,${(0, import_platejs7.hexToBase64)(
      rtfImage.hex
    )}`;
    if (element.tagName === "IMG") {
...
var cleanDocxListElements = (rootNode) => {
  (0, import_platejs8.traverseHtmlElements)(rootNode, (element) => {
...
var getDocxListContentHtml = (rootElement) => {
  ...
  (0, import_platejs9.removeHtmlNodesBetweenComments)(
    clonedElement,
    "[if !supportLists]",
    "[endif]"
  );
  (0, import_platejs9.traverseHtmlElements)(clonedElement, (element) => {
    const styleAttribute = element.getAttribute("style");
...
var isDocxOl = (element) => {
  ...
  return (0, import_platejs10.isOlSymbol)(listNode.textContent || "");
};
...
var docxListToList = (element) => {
  ...
  const list = (0, import_platejs11.parseHtmlElement)(`<${listTagName}>${listHtml}</${listTagName}>`);
  return { list, nextSibling };
};

// src/lib/docx-cleaner/utils/cleanDocxListElementsToList.ts
var cleanDocxListElementsToList = (rootNode) => {
  (0, import_platejs12.traverseHtmlElements)(rootNode, (element) => {
    const styleAttribute = element.getAttribute("style");
    if (styleAttribute) {
      element.setAttribute(
        "style",
...
var cleanDocxQuotes = (rootNode) => {
  (0, import_platejs13.traverseHtmlElements)(rootNode, (element) => {
    if (element.parentNode && element.tagName === "P" && element.classList.contains("MsoQuote")) {
      (0, import_platejs13.replaceTagName)(element, "blockquote");
    }
    return true;
  });
};
...
var generateSpaces = (count) => Array.from({ length: count }, () => import_platejs14.SPACE).join("");
var generateTabs = (count) => Array.from({ length: count }, () => import_platejs14.TAB).join("");
...
var cleanDocxSpans = (rootNode) => {
  (0, import_platejs15.traverseHtmlElements)(rootNode, (element) => {
...
var isDocxContent = (body) => {
  let result = false;
  (0, import_platejs16.traverseHtmlElements)(body, (element) => {
    const styleAttribute = element.getAttribute("style") || "";
...
var cleanDocx = (html, rtf) => {
  const document2 = new DOMParser().parseFromString(
    (0, import_platejs17.preCleanHtml)(html),
    "text/html"
  );
  ...
  (0, import_platejs17.cleanHtmlEmptyElements)(body);
  cleanDocxEmptyParagraphs(body);
  cleanDocxQuotes(body);
  cleanDocxSpans(body);
  (0, import_platejs17.cleanHtmlTextNodes)(body);
  cleanDocxBrComments(body);
  (0, import_platejs17.cleanHtmlBrElements)(body);
  (0, import_platejs17.cleanHtmlLinkElements)(body);
  (0, import_platejs17.cleanHtmlFontElements)(body);
  cleanDocxListElements(body);
  (0, import_platejs17.copyBlockMarksToSpanChild)(body);
  ...
  return (0, import_platejs17.postCleanHtml)(preformattedWrapper.outerHTML);
};
...
var DocxPlugin = (0, import_platejs18.createSlatePlugin)({
  key: import_platejs18.KEYS.docx,
  editOnly: true,
  inject: {
    plugins: {
      [import_platejs18.KEYS.html]: {
        parser: {
