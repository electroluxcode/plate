...
function BoundHotkeysProxyProviderProvider({
  ...
  return /* @__PURE__ */ import_react.default.createElement(BoundHotkeysProxyProvider.Provider, { value: { addHotkey, removeHotkey } }, children);
}
...
var HotkeysProvider = ({
  ...
  const [internalActiveScopes, setInternalActiveScopes] = (0, import_react2.useState)(
    initiallyActiveScopes
  );
  const [boundHotkeys, setBoundHotkeys] = (0, import_react2.useState)([]);
  const enableScope = (0, import_react2.useCallback)((scope) => {
    setInternalActiveScopes((prev) => {
      if (prev.includes("*")) {
        return [scope];
      }
      return Array.from(/* @__PURE__ */ new Set([scope, ...prev]));
    });
  }, []);
  const disableScope = (0, import_react2.useCallback)((scope) => {
    setInternalActiveScopes((prev) => {
      return prev.filter((s) => s !== scope);
    });
  }, []);
  const toggleScope = (0, import_react2.useCallback)((scope) => {
    setInternalActiveScopes((prev) => {
  ...
  const addBoundHotkey = (0, import_react2.useCallback)((hotkey) => {
    setBoundHotkeys((prev) => [...prev, hotkey]);
  }, []);
  const removeBoundHotkey = (0, import_react2.useCallback)((hotkey) => {
...
function isHotkeyEnabledOnTag({ target }, enabledOnTags = false) {
  ...
  if (isReadonlyArray(enabledOnTags)) {
    return Boolean(
      targetTagName && enabledOnTags?.some(
...
var isHotkeyMatchingKeyboardEvent = (e, hotkey, ignoreModifiers = false) => {
  ...
  if (!keys?.includes(mappedCode) && !["alt", "control", "ctrl", "meta", "os", "shift", "unknown"].includes(
    mappedCode
  )) {
    return false;
  }
...
var useSafeLayoutEffect = typeof window === "undefined" ? import_react4.useEffect : import_react4.useLayoutEffect;
function useHotkeys(keys, callback, options, dependencies) {
  const [ref, setRef] = (0, import_react4.useState)(null);
  const hasTriggeredRef = (0, import_react4.useRef)(false);
  const _options = Array.isArray(options) ? Array.isArray(dependencies) ? void 0 : dependencies : options;
  const _keys = (0, import_react4.useMemo)(() => {
    if (Array.isArray(keys) && keys.length > 0 && Array.isArray(keys[0])) {
      return keys.map(
        (keyCombo) => keyCombo.map((k) => k.toString()).join(_options?.splitKey || "+")
  ...
  const memoisedCB = (0, import_react4.useCallback)(callback, _deps ?? []);
  const cbRef = (0, import_react4.useRef)(memoisedCB);
  ...
  useSafeLayoutEffect(() => {
    ...
    const listener = (e, isKeyUp = false) => {
      ...
      if (e.target?.isContentEditable && !memoisedOptions?.enableOnContentEditable) {
        return;
      }
    ...
    const handleKeyDown = (event) => {
      ...
      if (memoisedOptions?.keydown === void 0 && memoisedOptions?.keyup !== true || memoisedOptions?.keydown) {
        listener(event);
      }
    };
    ...
    if (proxy) {
      parseKeysHookInput(_keys, memoisedOptions?.delimiter).forEach(
        (key) => proxy.addHotkey(
          parseHotkey(
            key,
            memoisedOptions?.splitKey,
...
function useRecordHotkeys() {
  const [keys, setKeys] = (0, import_react5.useState)(/* @__PURE__ */ new Set());
  const [isRecording, setIsRecording] = (0, import_react5.useState)(false);
  const handler = (0, import_react5.useCallback)((event) => {
    if (event.code === void 0) {
      return;
    }
  ...
  const resetKeys = (0, import_react5.useCallback)(() => {
    setKeys(/* @__PURE__ */ new Set());
  }, []);
