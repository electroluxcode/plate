...
var import_platejs45 = require("platejs");
...
var getEmptyCellNode = (editor, { children, header, row } = {}) => {
  header = header ?? (row ? row.children.every(
    (c) => c.type === editor.getType(import_platejs.KEYS.th)
  ) : false);
  return {
    children: children ?? [editor.api.create.block()],
    type: header ? editor.getType(import_platejs.KEYS.th) : editor.getType(import_platejs.KEYS.td)
  };
};
...
var getEmptyRowNode = (editor, { colCount = 1, ...cellOptions } = {}) => {
  const { api } = editor.getPlugin({ key: import_platejs2.KEYS.table });
  return {
    children: Array.from({ length: colCount }).fill(colCount).map(() => api.create.tableCell(cellOptions)),
    type: editor.getType(import_platejs2.KEYS.tr)
  };
};
...
var getEmptyTableNode = (editor, {
  ...
  const { api } = editor.getPlugin({ key: import_platejs3.KEYS.table });
...
function computeCellIndices(editor, {
  ...
  const { api, getOptions, setOption } = (0, import_platejs4.getEditorPlugin)(editor, {
    key: import_platejs4.KEYS.table
  });
  if (!tableNode) {
    if (!cellNode) return;
    tableNode = editor.api.above({
      at: cellNode,
      match: { type: editor.getType(import_platejs4.KEYS.table) }
    })?.[0];
    if (!tableNode) return;
  }
...
var getCellIndices = (editor, element) => {
  const { getOption } = (0, import_platejs5.getEditorPlugin)(editor, {
    key: import_platejs5.KEYS.table
  });
...
var getCellTypes = (editor) => (0, import_platejs6.getPluginTypes)(editor, [import_platejs6.KEYS.td, import_platejs6.KEYS.th]);
...
var getTableMergeGridByRange = (editor, { at, format }) => {
  const { api, type } = (0, import_platejs7.getEditorPlugin)(editor, BaseTablePlugin);
...
var getTableGridByRange = (editor, { at, format = "table" }) => {
  const { api } = editor.getPlugin({ key: import_platejs8.KEYS.table });
  ...
  while (true) {
    ...
    const cell = import_platejs8.NodeApi.get(editor, cellPath);
    if (!cell) break;
    const rows = table.children[rowIndex - startRowIndex].children;
    rows[colIndex - startColIndex] = cell;
    cellEntries.push([cell, cellPath]);
...
var getTableGridAbove = (editor, { format = "table", ...options } = {}) => {
  const { api } = editor.getPlugin({ key: import_platejs9.KEYS.table });
  ...
  if (edges) {
    ...
    if (!import_platejs9.PathApi.equals(start[1], end[1])) {
...
var moveSelectionFromCell = (editor, {
  ...
  if (edge) {
    ...
    if (cellEntries.length > minCell) {
      ...
      if (import_platejs10.NodeApi.has(editor, anchorPath) && import_platejs10.NodeApi.has(editor, focusPath)) {
        editor.tf.select({
  ...
  if (cellEntry) {
    ...
    if (import_platejs10.NodeApi.has(editor, nextCellPath)) {
...
var overrideSelectionFromCell = (editor, newSelection) => {
  let hotkey;
  if (!editor.dom.currentKeyboardEvent || !["up", "down", "shift+up", "shift+right", "shift+down", "shift+left"].some(
    (key) => {
      const valid = (0, import_platejs11.isHotkey)(key, editor.dom.currentKeyboardEvent);
      if (valid) hotkey = key;
      return valid;
    }
  ...
  transforms: {
    apply(op) {
      if (op.type === "set_selection" && op.newProperties) {
        ...
        if (import_platejs12.RangeApi.isRange(newSelection) && editor.api.isAt({
          at: newSelection,
          blocks: true,
          ...
          if (anchorEntry) {
            ...
            const isBackward = import_platejs12.RangeApi.isBackward(newSelection);
            if (isBackward) {
      ...
      const isTableOperation = (op.type === "remove_node" || op.type === "move_node") && opType && [editor.getType(import_platejs12.KEYS.tr), tableType, ...getCellTypes(editor)].includes(
        opType
      );
  ...
  transforms: {
    insertFragment(fragment) {
      ...
      if (insertedTable) {
            ...
            editor.tf.withoutNormalizing(() => {
              ...
              insertedRows.forEach((row) => {
                ...
                if (!initRow) {
                  ...
                  if (!import_platejs14.NodeApi.has(editor, cellPath)) {
        ...
        } else if (fragment.length === 1 && fragment[0].type === import_platejs14.KEYS.table) {
...
var getCellInNextTableRow = (editor, currentRowPath) => {
  const nextRow = editor.api.node(import_platejs15.PathApi.next(currentRowPath));
  if (!nextRow) return;
...
var getCellInPreviousTableRow = (editor, currentRowPath) => {
  const prevPath = import_platejs16.PathApi.previous(currentRowPath);
  if (!prevPath) return;
...
var getLeftTableCell = (editor, {
  ...
  if (!cellIndex) return;
  const prevCellPath = import_platejs17.PathApi.previous(cellPath);
...
var getNextTableCell = (editor, currentCell, currentPath, currentRow) => {
  const cell = editor.api.node(import_platejs18.PathApi.next(currentPath));
  if (cell) return cell;
...
var getPreviousTableCell = (editor, currentCell, currentPath, currentRow) => {
  const prevPath = import_platejs19.PathApi.previous(currentPath);
  if (!prevPath) {
    const [, currentRowPath] = currentRow;
...
var getTopTableCell = (editor, {
  ...
  const cellAbovePath = [
    ...import_platejs20.PathApi.parent(import_platejs20.PathApi.parent(cellPath)),
    rowIndex - 1,
    cellIndex
  ];
...
var getTableAbove = (editor, options) => editor.api.block({
  above: true,
  match: {
    type: editor.getType(import_platejs21.KEYS.table)
  },
  ...options
});
...
var getTableCellSize = (editor, {
  ...
  const { api } = (0, import_platejs22.getEditorPlugin)(editor, {
    key: import_platejs22.KEYS.table
  });
...
var getTableEntries = (editor, { at = editor.selection } = {}) => {
  ...
  const rowEntry = editor.api.above({
    at: cellPath,
    match: { type: editor.getType(import_platejs23.KEYS.tr) }
  });
  if (!rowEntry) return;
  const [, rowPath] = rowEntry;
  const tableEntry = editor.api.above({
    at: rowPath,
    match: { type: editor.getType(import_platejs23.KEYS.table) }
  });
...
// src/lib/withTableCellSelection.tsx
var import_platejs24 = require("platejs");
var withTableCellSelection = ({
  api: { marks },
  editor,
        ...
        const apply = () => {
          ...
          matchesCell.forEach(([_cell, cellPath]) => {
            const textNodeEntry = editor.api.nodes({
              at: cellPath,
              match: (n) => import_platejs24.TextApi.isText(n)
            });
      ...
      setNodes(props, options) {
        const apply = () => {
          ...
          if (options?.at) {
            ...
            if (range && !import_platejs24.RangeApi.includes(selection, range)) {
              return;
            }
          }
          const matchesCell = getTableGridAbove(editor, { format: "cell" });
          ...
          setNodes(props, {
            ...options,
            match: (0, import_platejs24.combineTransformMatchOptions)(
              editor,
              (_, p) => matchesCell.some(
                ([_2, cellPath]) => import_platejs24.PathApi.isCommon(cellPath, p)
              ),
              options
            )
          });
...
var withNormalizeTable = ({
  ...
  transforms: {
    normalizeNode([n, path]) {
      ...
      if (import_platejs25.ElementApi.isElement(n)) {
        if (n.type === type) {
          const node = n;
          if (!node.children.some(
            (child) => import_platejs25.ElementApi.isElement(child) && child.type === editor.getType(import_platejs25.KEYS.tr)
          )) {
        ...
        if (n.type === editor.getType(import_platejs25.KEYS.tr)) {
        ...
        if (getCellTypes(editor).includes(n.type)) {
          ...
          const { children } = node;
          const parentEntry = editor.api.parent(path);
          if (parentEntry?.[0].type !== editor.getType(import_platejs25.KEYS.tr)) {
            editor.tf.unwrapNodes({
              at: path
            });
            return;
          }
          if (import_platejs25.TextApi.isText(children[0])) {
    ...
    setFragmentData(data, originEvent) {
      ...
      editor.tf.withoutNormalizing(() => {
        tableRows.forEach((row) => {
          ...
          const cellStrings = [];
          const rowElement = row.type === editor.getType(import_platejs26.KEYS.th) ? document.createElement("th") : document.createElement("tr");
...
`;
        ...
        const _tableEntry = editor.api.node({
          at: tablePath,
          match: { type: import_platejs26.KEYS.table }
        });
...
var deleteColumnWhenExpanded = (editor, tableEntry) => {
  const [start, end] = import_platejs27.RangeApi.edges(editor.selection);
  const firstRow = import_platejs27.NodeApi.child(tableEntry[0], 0);
  const lastRow = import_platejs27.NodeApi.child(
    tableEntry[0],
    tableEntry[0].children.length - 1
  );
  const firstSelectionRow = editor.api.above({
    at: start,
    match: (n) => n.type === import_platejs27.KEYS.tr
  });
...
var deleteColumn = (editor) => {
  const { getOptions, type } = (0, import_platejs28.getEditorPlugin)(editor, {
    key: import_platejs28.KEYS.table
  });
  ...
  editor.tf.withoutNormalizing(() => {
    ...
    const trEntry = editor.api.above({
      match: { type: editor.getType(import_platejs28.KEYS.tr) }
    });
...
var deleteRowWhenExpanded = (editor, [table, tablePath]) => {
  const { api } = (0, import_platejs29.getEditorPlugin)(editor, BaseTablePlugin);
...
var deleteTableMergeRow = (editor) => {
  const { api, tf, type } = (0, import_platejs30.getEditorPlugin)(editor, {
    key: import_platejs30.KEYS.table
  });
  if (editor.api.some({
    match: { type }
  })) {
...
var deleteRow = (editor) => {
  const { getOptions, type } = (0, import_platejs31.getEditorPlugin)(editor, {
    key: import_platejs31.KEYS.table
  });
  const { disableMerge } = getOptions();
  if (!disableMerge) {
  ...
  if (editor.api.some({
    ...
    const currentRowItem = editor.api.above({
      match: { type: editor.getType(import_platejs31.KEYS.tr) }
    });
...
var deleteTable = (editor) => {
  if (editor.api.some({
    match: { type: editor.getType(import_platejs32.KEYS.table) }
  })) {
...
var import_platejs33 = require("platejs");
var insertTable = (editor, { colCount = 2, header, rowCount = 2 } = {}, { select: shouldSelect, ...options } = {}) => {
  const { api } = editor.getPlugin({ key: import_platejs33.KEYS.table });
  const type = editor.getType(import_platejs33.KEYS.table);
  editor.tf.withoutNormalizing(() => {
    ...
    if (!options.at) {
      ...
      if (currentTableEntry) {
        ...
        const insertPath = import_platejs33.PathApi.next(tablePath);
...
var insertTableMergeColumn = (editor, {
  ...
  const { api, getOptions, type } = (0, import_platejs34.getEditorPlugin)(editor, BaseTablePlugin);
  const { initialTableWidth, minColumnWidth } = getOptions();
  if (at && !fromCell) {
    const table = import_platejs34.NodeApi.get(editor, at);
    if (table?.type === editor.getType(import_platejs34.KEYS.table)) {
      fromCell = import_platejs34.NodeApi.lastChild(editor, at.concat([0]))[1];
      at = void 0;
    }
  }
  ...
  if (import_platejs34.PathApi.isPath(at)) {
...
var insertTableColumn = (editor, options = {}) => {
  const { api, getOptions, type } = (0, import_platejs35.getEditorPlugin)(editor, BaseTablePlugin);
  ...
  if (at && !fromCell) {
    const table = import_platejs35.NodeApi.get(editor, at);
    if (table?.type === editor.getType(import_platejs35.KEYS.table)) {
      fromCell = import_platejs35.NodeApi.lastChild(editor, at.concat([0]))[1];
      at = void 0;
    }
  }
  const cellEntry = editor.api.block({
  ...
  let nextCellPath;
  let nextColIndex;
  if (import_platejs35.PathApi.isPath(at)) {
    nextCellPath = at;
    nextColIndex = at.at(-1);
  } else {
    nextCellPath = before ? cellPath : import_platejs35.PathApi.next(cellPath);
  ...
  editor.tf.withoutNormalizing(() => {
    tableNode.children.forEach((row, rowIndex) => {
      ...
      const isHeaderRow = header === void 0 ? row.children.every(
        (c) => c.type === editor.getType(import_platejs35.KEYS.th)
      ) : header;
...
var insertTableMergeRow = (editor, {
  ...
  const { api, type } = (0, import_platejs36.getEditorPlugin)(editor, BaseTablePlugin);
  if (at && !fromRow) {
    const table = import_platejs36.NodeApi.get(editor, at);
    if (table?.type === editor.getType(import_platejs36.KEYS.table)) {
      fromRow = import_platejs36.NodeApi.lastChild(editor, at)[1];
      at = void 0;
    }
  }
  const trEntry = editor.api.block({
    at: fromRow,
    match: { type: editor.getType(import_platejs36.KEYS.tr) }
  });
  if (!trEntry) return;
  ...
  let nextRowPath;
  if (import_platejs36.PathApi.isPath(at)) {
    ...
    editor.tf.insertNodes(
      {
        ...
        type: editor.getType(import_platejs36.KEYS.tr)
      },
      {
        at: nextRowPath,
        select: false
...
var insertTableRow = (editor, options = {}) => {
  const { api, getOptions, type } = (0, import_platejs37.getEditorPlugin)(editor, BaseTablePlugin);
  const { disableMerge } = getOptions();
  if (!disableMerge) {
  ...
  if (at && !fromRow) {
    const table = import_platejs37.NodeApi.get(editor, at);
    if (table?.type === editor.getType(import_platejs37.KEYS.table)) {
      fromRow = import_platejs37.NodeApi.lastChild(editor, at)[1];
      at = void 0;
    }
  }
  const trEntry = editor.api.block({
    at: fromRow,
    match: { type: editor.getType(import_platejs37.KEYS.tr) }
  });
  if (!trEntry) return;
  ...
  const getEmptyRowNode2 = () => ({
    children: trNode.children.map((_, i) => {
      ...
      const isHeaderColumn = !hasSingleRow && tableEntry[0].children.every(
        (n) => n.children[i].type === editor.getType(import_platejs37.KEYS.th)
      );
      return api.create.tableCell({
        header: header ?? isHeaderColumn
      });
    }),
    type: editor.getType(import_platejs37.KEYS.tr)
  });
  editor.tf.withoutNormalizing(() => {
    editor.tf.insertNodes(getEmptyRowNode2(), {
      at: import_platejs37.PathApi.isPath(at) ? at : before ? trPath : import_platejs37.PathApi.next(trPath)
    });
  });
  if (shouldSelect) {
    const cellEntry = editor.api.block({
    ...
    if (import_platejs37.PathApi.isPath(at)) {
...
var import_platejs38 = require("platejs");
var setBorderSize = (editor, size, {
  ...
  const setNodesOptions = {
    match: (n) => import_platejs38.ElementApi.isElement(n) && getCellTypes(editor).includes(n.type)
  };
  if (border === "top") {
...
var import_platejs40 = require("platejs");
var setTableMarginLeft = (editor, { marginLeft }, options = {}) => {
  const table = editor.api.node({
    match: { type: import_platejs40.KEYS.table },
    ...options
  });
  if (!table) return;
  const [, tablePath] = table;
...
var import_platejs41 = require("platejs");
var setTableRowSize = (editor, { height, rowIndex }, options = {}) => {
  const table = editor.api.node({
    match: { type: import_platejs41.KEYS.table },
    ...options
  });
  if (!table) return;
  const [, tablePath] = table;
...
var deleteTableMergeColumn = (editor) => {
  const type = editor.getType(import_platejs42.KEYS.table);
  const tableEntry = editor.api.above({
    match: { type }
  });
  if (!tableEntry) return;
  editor.tf.withoutNormalizing(() => {
    const { api } = (0, import_platejs42.getEditorPlugin)(editor, BaseTablePlugin);
    if (editor.api.isExpanded()) {
      return deleteColumnWhenExpanded(editor, tableEntry);
    }
    ...
    const trEntry = editor.api.above({
      match: { type: editor.getType(import_platejs42.KEYS.tr) }
    });
...
var import_platejs43 = require("platejs");
var mergeTableCells = (editor) => {
  const { api } = (0, import_platejs43.getEditorPlugin)(editor, BaseTablePlugin);
  const cellEntries = getTableGridAbove(editor, {
    format: "cell"
  });
  editor.tf.withoutNormalizing(() => {
    ...
    const mergedCell = {
      ...api.create.tableCell({
        ...
        header: cellEntries[0][0].type === editor.getType(import_platejs43.KEYS.th)
      }),
      colSpan,
      rowSpan
    };
...
var import_platejs44 = require("platejs");
var splitTableCell = (editor) => {
  const { api } = (0, import_platejs44.getEditorPlugin)(editor, BaseTablePlugin);
  const tableRowType = editor.getType(import_platejs44.KEYS.tr);
  const cellEntries = getTableGridAbove(editor, { format: "cell" });
  ...
  editor.tf.withoutNormalizing(() => {
    const createEmptyCell = (children) => {
      return {
        ...api.create.tableCell({
          children,
          header: cellElem.type === editor.getType(import_platejs44.KEYS.th)
        }),
        colSpan: 1,
        rowSpan: 1
      };
    ...
    for (let i = 0; i < rowSpan; i++) {
      ...
      if (!rowEntry) {
        editor.tf.insertNodes(
          {
            children: newRowChildren,
            type: editor.getType(import_platejs44.KEYS.tr)
          },
          { at: _rowPath }
        );
      }
...
var BaseTableRowPlugin = (0, import_platejs45.createSlatePlugin)({
  key: import_platejs45.KEYS.tr,
...
var BaseTableCellPlugin = (0, import_platejs45.createSlatePlugin)({
  key: import_platejs45.KEYS.td,
  node: {
...
var BaseTableCellHeaderPlugin = (0, import_platejs45.createSlatePlugin)({
  key: import_platejs45.KEYS.th,
  node: {
    dangerouslyAllowAttributes: ["colspan", "rowspan"],
    isContainer: true,
    isElement: true,
...
var BaseTablePlugin = (0, import_platejs45.createTSlatePlugin)({
  key: import_platejs45.KEYS.table,
  node: {
    isContainer: true,
    isElement: true
  },
  ...
  create: {
    table: (0, import_platejs45.bindFirst)(getEmptyTableNode, editor),
    tableCell: (0, import_platejs45.bindFirst)(getEmptyCellNode, editor),
    tableRow: (0, import_platejs45.bindFirst)(getEmptyRowNode, editor)
  },
  table: {
    getCellBorders: (0, import_platejs45.bindFirst)(getTableCellBorders, editor),
    getCellSize: (0, import_platejs45.bindFirst)(getTableCellSize, editor),
    getColSpan,
    getRowSpan,
  ...
  insert: {
    table: (0, import_platejs45.bindFirst)(insertTable, editor),
    tableColumn: (0, import_platejs45.bindFirst)(insertTableColumn, editor),
    tableRow: (0, import_platejs45.bindFirst)(insertTableRow, editor)
  },
  remove: {
    table: (0, import_platejs45.bindFirst)(deleteTable, editor),
    tableColumn: (0, import_platejs45.bindFirst)(deleteColumn, editor),
    tableRow: (0, import_platejs45.bindFirst)(deleteRow, editor)
  },
  table: {
    merge: (0, import_platejs45.bindFirst)(mergeTableCells, editor),
    split: (0, import_platejs45.bindFirst)(splitTableCell, editor)
  }
})).overrideEditor(withTable);

// src/react/onKeyDownTable.ts
var import_platejs46 = require("platejs");
var onKeyDownTable = ({
  editor,
  event
}) => {
  ...
  const isKeyDown = {
    "shift+down": import_platejs46.Hotkeys.isExtendDownward(event),
    "shift+left": import_platejs46.Hotkeys.isExtendBackward(event),
    "shift+right": import_platejs46.Hotkeys.isExtendForward(event),
    "shift+up": import_platejs46.Hotkeys.isExtendUpward(event)
  };
...
var import_react17 = require("platejs/react");
...
var import_platejs48 = require("platejs");
...
var useSelectedCells = () => {
  ...
  const selection = (0, import_react11.useEditorSelection)();
...
var import_platejs47 = require("platejs");
...
var useTableColSizes = ({
  ...
  const overriddenColSizes = (0, import_react12.useElementSelector)(
    ...
    {
      key: import_platejs47.KEYS.table,
      equalityFn: (a, b) => !!a && !!b && import_platejs47.PathApi.equals(a, b)
    }
  );
...
var import_resizable = require("@platejs/resizable");
var import_platejs49 = require("platejs");
var import_react19 = require("platejs/react");
var useTableCellElementResizable = ({
  ...
  const initialWidth = (0, import_react19.useElementSelector)(
    ...
    [colSpan, colIndex],
    { key: import_platejs49.KEYS.table }
  );
  ...
  const handleResizeRight = import_react18.default.useCallback(
    ({ delta, finished, initialSize: currentInitial }) => {
      ...
      const currentNew = roundCellSizeToStep(
        (0, import_resizable.resizeLengthClampStatic)(currentInitial + delta, {
  ...
  const handleResizeLeft = import_react18.default.useCallback(
    (event) => {
      ...
      const newMargin = roundCellSizeToStep(
        (0, import_resizable.resizeLengthClampStatic)(marginLeft + event.delta, {
...
var import_platejs50 = require("platejs");
...
var useTableMergeState = () => {
  ...
  const someTable = (0, import_react20.useEditorSelector)(
    (editor) => editor.api.some({ match: { type: import_platejs50.KEYS.table } }),
    []
  );
