...
var getEmptyTableNode = (editor, {
  ...
  const { api } = editor.getPlugin({ key: KEYS3.table });
  ...
  return {
    children: rows,
    type: editor.getType(KEYS3.table)
  };
};

// src/lib/merge/deleteColumn.ts
import cloneDeep5 from "lodash/cloneDeep.js";
import { getEditorPlugin as getEditorPlugin13, KEYS as KEYS29 } from "platejs";
...
function computeCellIndices(editor, {
  ...
  if (!tableNode) {
    ...
    tableNode = editor.api.above({
      at: cellNode,
      match: { type: editor.getType(KEYS4.table) }
    })?.[0];
    if (!tableNode) return;
  }
...
import { getPluginTypes, KEYS as KEYS6 } from "platejs";
var getCellTypes = (editor) => getPluginTypes(editor, [KEYS6.td, KEYS6.th]);
...
var getTableGridByRange = (editor, { at, format = "table" }) => {
  const { api } = editor.getPlugin({ key: KEYS7.table });
...
var getTableGridAbove = (editor, { format = "table", ...options } = {}) => {
  const { api } = editor.getPlugin({ key: KEYS8.table });
...
var withApplyTable = ({
  ...
  transforms: {
    apply(op) {
      ...
      const isTableOperation = (op.type === "remove_node" || op.type === "move_node") && opType && [editor.getType(KEYS9.tr), tableType, ...getCellTypes(editor)].includes(
        opType
      );
  ...
  transforms: {
    insertFragment(fragment) {
      ...
      if (insertedTable) {
        ...
        } else if (fragment.length === 1 && fragment[0].type === KEYS10.table) {
...
import { PathApi as PathApi4 } from "platejs";
...
import { PathApi as PathApi5 } from "platejs";
...
import { PathApi as PathApi6 } from "platejs";
...
var getTableAbove = (editor, options) => editor.api.block({
  above: true,
  match: {
    type: editor.getType(KEYS11.table)
  },
  ...options
});
...
import { getEditorPlugin as getEditorPlugin4, KEYS as KEYS12 } from "platejs";
...
var getTableEntries = (editor, { at = editor.selection } = {}) => {
  ...
  const rowEntry = editor.api.above({
    at: cellPath,
    match: { type: editor.getType(KEYS13.tr) }
  });
  if (!rowEntry) return;
  const [, rowPath] = rowEntry;
  const tableEntry = editor.api.above({
    at: rowPath,
    match: { type: editor.getType(KEYS13.table) }
  });
      ...
      setNodes(props, options) {
        const apply = () => {
          ...
          if (options?.at) {
            ...
            if (range && !RangeApi2.includes(selection, range)) {
              return;
            }
          }
          const matchesCell = getTableGridAbove(editor, { format: "cell" });
          ...
          setNodes(props, {
            ...
            match: combineTransformMatchOptions(
              ...
              (_, p) => matchesCell.some(
                ([_2, cellPath]) => PathApi8.isCommon(cellPath, p)
              ),
              options
            )
          });
      ...
      if (ElementApi.isElement(n)) {
        if (n.type === type) {
          ...
          if (!node.children.some(
            (child) => ElementApi.isElement(child) && child.type === editor.getType(KEYS14.tr)
          )) {
        ...
        if (n.type === editor.getType(KEYS14.tr)) {
        ...
        if (getCellTypes(editor).includes(n.type)) {
          ...
          const { children } = node;
          const parentEntry = editor.api.parent(path);
          if (parentEntry?.[0].type !== editor.getType(KEYS14.tr)) {
            editor.tf.unwrapNodes({
              at: path
            });
            return;
    ...
    setFragmentData(data, originEvent) {
      ...
      editor.tf.withoutNormalizing(() => {
        tableRows.forEach((row) => {
          ...
          const cellStrings = [];
          const rowElement = row.type === editor.getType(KEYS15.th) ? document.createElement("th") : document.createElement("tr");
...
import { getEditorPlugin as getEditorPlugin5, KEYS as KEYS17 } from "platejs";

// src/lib/merge/deleteColumnWhenExpanded.ts
import {
  KEYS as KEYS16,
  NodeApi as NodeApi4,
  RangeApi as RangeApi3
} from "platejs";
var deleteColumnWhenExpanded = (editor, tableEntry) => {
  const [start, end] = RangeApi3.edges(editor.selection);
...
var deleteColumn = (editor) => {
  ...
  editor.tf.withoutNormalizing(() => {
    ...
    const trEntry = editor.api.above({
      match: { type: editor.getType(KEYS17.tr) }
    });
...
import { getEditorPlugin as getEditorPlugin8, KEYS as KEYS19 } from "platejs";

// src/lib/merge/deleteRow.ts
import cloneDeep2 from "lodash/cloneDeep.js";
import { getEditorPlugin as getEditorPlugin7, KEYS as KEYS18 } from "platejs";

// src/lib/merge/deleteRowWhenExpanded.ts
import { getEditorPlugin as getEditorPlugin6 } from "platejs";
...
var deleteRow = (editor) => {
  ...
  if (editor.api.some({
    ...
    const currentRowItem = editor.api.above({
      match: { type: editor.getType(KEYS19.tr) }
    });
...
import { KEYS as KEYS20 } from "platejs";
var deleteTable = (editor) => {
  if (editor.api.some({
    match: { type: editor.getType(KEYS20.table) }
  })) {
...
var insertTable = (editor, { colCount = 2, header, rowCount = 2 } = {}, { select: shouldSelect, ...options } = {}) => {
  const { api } = editor.getPlugin({ key: KEYS21.table });
  const type = editor.getType(KEYS21.table);
  editor.tf.withoutNormalizing(() => {
    ...
    if (!options.at) {
      ...
      if (currentTableEntry) {
        ...
        const insertPath = PathApi9.next(tablePath);
...
import { getEditorPlugin as getEditorPlugin10, KEYS as KEYS23, NodeApi as NodeApi6, PathApi as PathApi11 } from "platejs";
...
var insertTableMergeColumn = (editor, {
  ...
  if (at && !fromCell) {
    ...
    if (table?.type === editor.getType(KEYS22.table)) {
...
var insertTableColumn = (editor, options = {}) => {
  ...
  if (at && !fromCell) {
    ...
    if (table?.type === editor.getType(KEYS23.table)) {
  ...
  if (PathApi11.isPath(at)) {
    ...
    nextCellPath = before ? cellPath : PathApi11.next(cellPath);
  ...
  editor.tf.withoutNormalizing(() => {
    tableNode.children.forEach((row, rowIndex) => {
      ...
      const isHeaderRow = header === void 0 ? row.children.every(
        (c) => c.type === editor.getType(KEYS23.th)
      ) : header;
...
var insertTableMergeRow = (editor, {
  ...
  if (at && !fromRow) {
    ...
    if (table?.type === editor.getType(KEYS24.table)) {
  ...
  const trEntry = editor.api.block({
    at: fromRow,
    match: { type: editor.getType(KEYS24.tr) }
  });
  if (!trEntry) return;
...
var insertTableRow = (editor, options = {}) => {
  ...
  if (at && !fromRow) {
    ...
    if (table?.type === editor.getType(KEYS25.table)) {
  ...
  const trEntry = editor.api.block({
    at: fromRow,
    match: { type: editor.getType(KEYS25.tr) }
  });
  if (!trEntry) return;
  ...
  const getEmptyRowNode2 = () => ({
    children: trNode.children.map((_, i) => {
      ...
      const isHeaderColumn = !hasSingleRow && tableEntry[0].children.every(
        (n) => n.children[i].type === editor.getType(KEYS25.th)
      );
      return api.create.tableCell({
        header: header ?? isHeaderColumn
      });
    }),
    type: editor.getType(KEYS25.tr)
  });
  editor.tf.withoutNormalizing(() => {
...
import { KEYS as KEYS27 } from "platejs";
var setTableMarginLeft = (editor, { marginLeft }, options = {}) => {
  const table = editor.api.node({
    match: { type: KEYS27.table },
    ...options
  });
  if (!table) return;
  const [, tablePath] = table;
...
import { KEYS as KEYS28 } from "platejs";
var setTableRowSize = (editor, { height, rowIndex }, options = {}) => {
  const table = editor.api.node({
    match: { type: KEYS28.table },
    ...options
  });
  if (!table) return;
  const [, tablePath] = table;
...
var deleteTableMergeColumn = (editor) => {
  const type = editor.getType(KEYS29.table);
  const tableEntry = editor.api.above({
    match: { type }
  });
  if (!tableEntry) return;
  editor.tf.withoutNormalizing(() => {
    ...
    const trEntry = editor.api.above({
      match: { type: editor.getType(KEYS29.tr) }
    });
...
import { getEditorPlugin as getEditorPlugin14, KEYS as KEYS30 } from "platejs";
  ...
  editor.tf.withoutNormalizing(() => {
    ...
    const mergedCell = {
      ...api.create.tableCell({
        ...
        header: cellEntries[0][0].type === editor.getType(KEYS30.th)
      }),
      colSpan,
      rowSpan
    };
...
import { getEditorPlugin as getEditorPlugin15, KEYS as KEYS31 } from "platejs";
var splitTableCell = (editor) => {
  const { api } = getEditorPlugin15(editor, BaseTablePlugin);
  const tableRowType = editor.getType(KEYS31.tr);
  const cellEntries = getTableGridAbove(editor, { format: "cell" });
  ...
  editor.tf.withoutNormalizing(() => {
    const createEmptyCell = (children) => {
      return {
        ...api.create.tableCell({
          children,
          header: cellElem.type === editor.getType(KEYS31.th)
        }),
        colSpan: 1,
        rowSpan: 1
      };
...
import { useEditorPlugin as useEditorPlugin3, useElement as useElement3 } from "platejs/react";

// src/react/hooks/useCellIndices.ts
import React from "react";
import { useEditorPlugin as useEditorPlugin2, useElement as useElement2, usePluginOption as usePluginOption3 } from "platejs/react";
var useCellIndices = () => {
...
import { useEditorPlugin as useEditorPlugin7, useElement as useElement6, usePluginOption as usePluginOption6 } from "platejs/react";
...
import { KEYS as KEYS34 } from "platejs";
import { useEditorPlugin as useEditorPlugin6, useElement as useElement5, useElementSelector as useElementSelector2 } from "platejs/react";
...
import { KEYS as KEYS33, PathApi as PathApi14 } from "platejs";
...
var useTableColSizes = ({
  ...
  const overriddenColSizes = useElementSelector(
    ...
    {
      key: KEYS33.table,
      equalityFn: (a, b) => !!a && !!b && PathApi14.equals(a, b)
    }
  );
...
import { useEditorPlugin as useEditorPlugin5, useElement as useElement4, usePluginOption as usePluginOption5 } from "platejs/react";
var useTableElement = () => {
...
import { KEYS as KEYS35 } from "platejs";
import { useEditorPlugin as useEditorPlugin8, useElement as useElement7, useElementSelector as useElementSelector3 } from "platejs/react";
...
import { KEYS as KEYS36 } from "platejs";
import {
...
var useTableMergeState = () => {
  ...
  const someTable = useEditorSelector2(
    (editor) => editor.api.some({ match: { type: KEYS36.table } }),
    []
  );
