...
__export(index_exports, {
  ...
  getSelectedCellsBorders: () => getSelectedCellsBorders,
  getSelectedCellsBoundingBox: () => getSelectedCellsBoundingBox,
  getSelectionWidth: () => getSelectionWidth,
  getTableAbove: () => getTableAbove,
  getTableCellBorders: () => getTableCellBorders,
  getTableCellSize: () => getTableCellSize,
  ...
  isSelectedCellBorder: () => isSelectedCellBorder,
  isSelectedCellBordersNone: () => isSelectedCellBordersNone,
  isSelectedCellBordersOuter: () => isSelectedCellBordersOuter,
  ...
  normalizeInitialValueTable: () => normalizeInitialValueTable,
  ...
  withTable: () => withTable,
  withTableCellSelection: () => withTableCellSelection
});
...
var getCellInNextTableRow = (editor, currentRowPath) => {
  const nextRow = editor.api.node(import_platejs4.PathApi.next(currentRowPath));
  if (!nextRow) return;
...
var getCellInPreviousTableRow = (editor, currentRowPath) => {
  const prevPath = import_platejs5.PathApi.previous(currentRowPath);
  if (!prevPath) return;
...
function computeCellIndices(editor, {
  ...
  const { api, getOptions, setOption } = (0, import_platejs6.getEditorPlugin)(editor, {
    key: import_platejs6.KEYS.table
  });
  if (!tableNode) {
    if (!cellNode) return;
    tableNode = editor.api.above({
      at: cellNode,
      match: { type: editor.getType(import_platejs6.KEYS.table) }
    })?.[0];
    if (!tableNode) return;
  }
...
var getCellIndices = (editor, element) => {
  const { getOption } = (0, import_platejs7.getEditorPlugin)(editor, {
    key: import_platejs7.KEYS.table
  });
...
var getCellTypes = (editor) => (0, import_platejs8.getPluginTypes)(editor, [import_platejs8.KEYS.td, import_platejs8.KEYS.th]);
...
var getLeftTableCell = (editor, {
  ...
  if (!cellIndex) return;
  const prevCellPath = import_platejs9.PathApi.previous(cellPath);
...
var getNextTableCell = (editor, currentCell, currentPath, currentRow) => {
  const cell = editor.api.node(import_platejs10.PathApi.next(currentPath));
  if (cell) return cell;
...
var getPreviousTableCell = (editor, currentCell, currentPath, currentRow) => {
  const prevPath = import_platejs11.PathApi.previous(currentPath);
  if (!prevPath) {
    const [, currentRowPath] = currentRow;
...
var getTopTableCell = (editor, {
  ...
  const cellAbovePath = [
    ...import_platejs12.PathApi.parent(import_platejs12.PathApi.parent(cellPath)),
    rowIndex - 1,
    cellIndex
  ];
...
var getTableAbove = (editor, options) => editor.api.block({
  above: true,
  match: {
    type: editor.getType(import_platejs13.KEYS.table)
  },
  ...options
});
...
var getTableCellSize = (editor, {
  ...
  const { api } = (0, import_platejs14.getEditorPlugin)(editor, {
    key: import_platejs14.KEYS.table
  });
...
var getTableEntries = (editor, { at = editor.selection } = {}) => {
  ...
  const rowEntry = editor.api.above({
    at: cellPath,
    match: { type: editor.getType(import_platejs15.KEYS.tr) }
  });
  if (!rowEntry) return;
  const [, rowPath] = rowEntry;
  const tableEntry = editor.api.above({
    at: rowPath,
    match: { type: editor.getType(import_platejs15.KEYS.table) }
  });
...
var getTableMergeGridByRange = (editor, { at, format }) => {
  const { api, type } = (0, import_platejs16.getEditorPlugin)(editor, BaseTablePlugin);
...
var getTableGridByRange = (editor, { at, format = "table" }) => {
  const { api } = editor.getPlugin({ key: import_platejs17.KEYS.table });
  ...
  while (true) {
    ...
    const cell = import_platejs17.NodeApi.get(editor, cellPath);
    if (!cell) break;
    const rows = table.children[rowIndex - startRowIndex].children;
    rows[colIndex - startColIndex] = cell;
    cellEntries.push([cell, cellPath]);
...
var getTableGridAbove = (editor, { format = "table", ...options } = {}) => {
  const { api } = editor.getPlugin({ key: import_platejs18.KEYS.table });
  ...
  if (edges) {
    ...
    if (!import_platejs18.PathApi.equals(start[1], end[1])) {
...
var getTableRowIndex = (editor, cellNode) => {
  const path = editor.api.findPath(cellNode);
  if (!path) return 0;
  const rowPath = import_platejs19.PathApi.parent(path);
  return rowPath.at(-1);
};
...
var deleteColumnWhenExpanded = (editor, tableEntry) => {
  const [start, end] = import_platejs20.RangeApi.edges(editor.selection);
  const firstRow = import_platejs20.NodeApi.child(tableEntry[0], 0);
  const lastRow = import_platejs20.NodeApi.child(
    tableEntry[0],
    tableEntry[0].children.length - 1
  );
  const firstSelectionRow = editor.api.above({
    at: start,
    match: (n) => n.type === import_platejs20.KEYS.tr
  });
...
var deleteTableMergeColumn = (editor) => {
  const type = editor.getType(import_platejs21.KEYS.table);
  const tableEntry = editor.api.above({
    match: { type }
  });
  if (!tableEntry) return;
  editor.tf.withoutNormalizing(() => {
    const { api } = (0, import_platejs21.getEditorPlugin)(editor, BaseTablePlugin);
    ...
    squizeColSpanCells.forEach((cur) => {
      ...
      const newCell = (0, import_cloneDeep.default)({ ...curCell, colSpan });
    ...
    const trEntry = editor.api.above({
      match: { type: editor.getType(import_platejs21.KEYS.tr) }
    });
...
var deleteRowWhenExpanded = (editor, [table, tablePath]) => {
  const { api } = (0, import_platejs22.getEditorPlugin)(editor, BaseTablePlugin);
...
var deleteTableMergeRow = (editor) => {
  const { api, tf, type } = (0, import_platejs23.getEditorPlugin)(editor, {
    key: import_platejs23.KEYS.table
  });
  if (editor.api.some({
    match: { type }
  })) {
...
var insertTableMergeColumn = (editor, {
  ...
  const { api, getOptions, type } = (0, import_platejs24.getEditorPlugin)(editor, BaseTablePlugin);
  const { initialTableWidth, minColumnWidth } = getOptions();
  if (at && !fromCell) {
    const table = import_platejs24.NodeApi.get(editor, at);
    if (table?.type === editor.getType(import_platejs24.KEYS.table)) {
      fromCell = import_platejs24.NodeApi.lastChild(editor, at.concat([0]))[1];
      at = void 0;
    }
  }
  ...
  if (import_platejs24.PathApi.isPath(at)) {
...
var insertTableMergeRow = (editor, {
  ...
  const { api, type } = (0, import_platejs25.getEditorPlugin)(editor, BaseTablePlugin);
  if (at && !fromRow) {
    const table = import_platejs25.NodeApi.get(editor, at);
    if (table?.type === editor.getType(import_platejs25.KEYS.table)) {
      fromRow = import_platejs25.NodeApi.lastChild(editor, at)[1];
      at = void 0;
    }
  }
  const trEntry = editor.api.block({
    at: fromRow,
    match: { type: editor.getType(import_platejs25.KEYS.tr) }
  });
  if (!trEntry) return;
  ...
  let nextRowPath;
  if (import_platejs25.PathApi.isPath(at)) {
    ...
    editor.tf.insertNodes(
      {
        ...
        type: editor.getType(import_platejs25.KEYS.tr)
      },
      {
        at: nextRowPath,
        select: false
...
var mergeTableCells = (editor) => {
  const { api } = (0, import_platejs26.getEditorPlugin)(editor, BaseTablePlugin);
  ...
  editor.tf.withoutNormalizing(() => {
    ...
    for (const cellEntry of cellEntries) {
      ...
      if (cellChildren.length !== 1 || !editor.api.isEmpty(cellChildren[0])) {
        mergingCellChildren.push(...(0, import_cloneDeep5.default)(cellChildren));
      }
    }
    const cols = {};
    ...
    const mergedCell = {
      ...api.create.tableCell({
        ...
        header: cellEntries[0][0].type === editor.getType(import_platejs26.KEYS.th)
      }),
      colSpan,
      rowSpan
    };
...
var splitTableCell = (editor) => {
  const { api } = (0, import_platejs27.getEditorPlugin)(editor, BaseTablePlugin);
  const tableRowType = editor.getType(import_platejs27.KEYS.tr);
  ...
  editor.tf.withoutNormalizing(() => {
    const createEmptyCell = (children) => {
      return {
        ...api.create.tableCell({
          children,
          header: cellElem.type === editor.getType(import_platejs27.KEYS.th)
        }),
        colSpan: 1,
        rowSpan: 1
      };
    ...
    for (let i = 0; i < rowSpan; i++) {
      ...
      if (!rowEntry) {
        editor.tf.insertNodes(
          {
            children: newRowChildren,
            type: editor.getType(import_platejs27.KEYS.tr)
          },
          { at: _rowPath }
        );
      }
...
var deleteRow = (editor) => {
  const { getOptions, type } = (0, import_platejs29.getEditorPlugin)(editor, {
    key: import_platejs29.KEYS.table
  });
  const { disableMerge } = getOptions();
  if (!disableMerge) {
  ...
  if (editor.api.some({
    ...
    const currentRowItem = editor.api.above({
      match: { type: editor.getType(import_platejs29.KEYS.tr) }
    });
...
var deleteTable = (editor) => {
  if (editor.api.some({
    match: { type: editor.getType(import_platejs30.KEYS.table) }
  })) {
...
var insertTable = (editor, { colCount = 2, header, rowCount = 2 } = {}, { select: shouldSelect, ...options } = {}) => {
  const { api } = editor.getPlugin({ key: import_platejs31.KEYS.table });
  const type = editor.getType(import_platejs31.KEYS.table);
  editor.tf.withoutNormalizing(() => {
    ...
    if (!options.at) {
      ...
      if (currentTableEntry) {
        ...
        const insertPath = import_platejs31.PathApi.next(tablePath);
...
var insertTableColumn = (editor, options = {}) => {
  const { api, getOptions, type } = (0, import_platejs32.getEditorPlugin)(editor, BaseTablePlugin);
  ...
  if (at && !fromCell) {
    const table = import_platejs32.NodeApi.get(editor, at);
    if (table?.type === editor.getType(import_platejs32.KEYS.table)) {
      fromCell = import_platejs32.NodeApi.lastChild(editor, at.concat([0]))[1];
      at = void 0;
    }
  }
  ...
  let nextCellPath;
  let nextColIndex;
  if (import_platejs32.PathApi.isPath(at)) {
    nextCellPath = at;
    nextColIndex = at.at(-1);
  } else {
    nextCellPath = before ? cellPath : import_platejs32.PathApi.next(cellPath);
  ...
  editor.tf.withoutNormalizing(() => {
    tableNode.children.forEach((row, rowIndex) => {
      ...
      const isHeaderRow = header === void 0 ? row.children.every(
        (c) => c.type === editor.getType(import_platejs32.KEYS.th)
      ) : header;
      editor.tf.insertNodes(
        api.create.tableCell({
          header: isHeaderRow
...
var insertTableRow = (editor, options = {}) => {
  const { api, getOptions, type } = (0, import_platejs33.getEditorPlugin)(editor, BaseTablePlugin);
  const { disableMerge } = getOptions();
  if (!disableMerge) {
  ...
  if (at && !fromRow) {
    const table = import_platejs33.NodeApi.get(editor, at);
    if (table?.type === editor.getType(import_platejs33.KEYS.table)) {
      fromRow = import_platejs33.NodeApi.lastChild(editor, at)[1];
      at = void 0;
    }
  }
  const trEntry = editor.api.block({
    at: fromRow,
    match: { type: editor.getType(import_platejs33.KEYS.tr) }
  });
  if (!trEntry) return;
  ...
  const getEmptyRowNode2 = () => ({
    children: trNode.children.map((_, i) => {
      ...
      const isHeaderColumn = !hasSingleRow && tableEntry[0].children.every(
        (n) => n.children[i].type === editor.getType(import_platejs33.KEYS.th)
      );
      return api.create.tableCell({
        header: header ?? isHeaderColumn
      });
    }),
    type: editor.getType(import_platejs33.KEYS.tr)
  });
  editor.tf.withoutNormalizing(() => {
    editor.tf.insertNodes(getEmptyRowNode2(), {
      at: import_platejs33.PathApi.isPath(at) ? at : before ? trPath : import_platejs33.PathApi.next(trPath)
    });
  });
  if (shouldSelect) {
    const cellEntry = editor.api.block({
    ...
    if (import_platejs33.PathApi.isPath(at)) {
...
var moveSelectionFromCell = (editor, {
  ...
  if (edge) {
    ...
    if (cellEntries.length > minCell) {
      ...
      if (import_platejs34.NodeApi.has(editor, anchorPath) && import_platejs34.NodeApi.has(editor, focusPath)) {
        editor.tf.select({
  ...
  if (cellEntry) {
    ...
    if (import_platejs34.NodeApi.has(editor, nextCellPath)) {
...
var overrideSelectionFromCell = (editor, newSelection) => {
  ...
  if (!editor.dom.currentKeyboardEvent || !["up", "down", "shift+up", "shift+right", "shift+down", "shift+left"].some(
    (key) => {
      const valid = (0, import_platejs35.isHotkey)(key, editor.dom.currentKeyboardEvent);
      if (valid) hotkey = key;
      return valid;
    }
...
var setBorderSize = (editor, size, {
  ...
  const setNodesOptions = {
    match: (n) => import_platejs36.ElementApi.isElement(n) && getCellTypes(editor).includes(n.type)
  };
  if (border === "top") {
...
var setTableColSize = (editor, { colIndex, width }, options = {}) => {
  const table = editor.api.node({
    match: { type: import_platejs37.KEYS.table },
    ...options
  });
  if (!table) return;
...
var setTableMarginLeft = (editor, { marginLeft }, options = {}) => {
  const table = editor.api.node({
    match: { type: import_platejs38.KEYS.table },
    ...options
  });
  if (!table) return;
  ...
  transforms: {
    apply(op) {
      if (op.type === "set_selection" && op.newProperties) {
        ...
        if (import_platejs40.RangeApi.isRange(newSelection) && editor.api.isAt({
          at: newSelection,
          blocks: true,
          ...
          if (anchorEntry) {
            ...
            const isBackward = import_platejs40.RangeApi.isBackward(newSelection);
            if (isBackward) {
      ...
      const isTableOperation = (op.type === "remove_node" || op.type === "move_node") && opType && [editor.getType(import_platejs40.KEYS.tr), tableType, ...getCellTypes(editor)].includes(
        opType
      );
...
var preventDeleteTableCell = (editor, {
  ...
  if (editor.api.isCollapsed()) {
    ...
    if (cellEntry) {
      ...
      if (selection && import_platejs41.PointApi.equals(selection.anchor, start)) {
        return true;
      }
    } else {
  ...
  transforms: {
    insertFragment(fragment) {
        ...
        if (tableEntry) {
          ...
          if (cellEntries.length > 1) {
            cellEntries.forEach((cellEntry) => {
              if (cellEntry) {
                const [, cellPath] = cellEntry;
                editor.tf.replaceNodes((0, import_cloneDeep6.default)(fragment), {
                  at: cellPath,
                  children: true
                });
              }
      ...
      if (insertedTable) {
            ...
            editor.tf.withoutNormalizing(() => {
              ...
              insertedRows.forEach((row) => {
                ...
                if (!initRow) {
                  ...
                  if (!import_platejs42.NodeApi.has(editor, cellPath)) {
                ...
                insertedCells.forEach((cell) => {
                  ...
                  editor.tf.replaceNodes((0, import_cloneDeep6.default)(cellChildren), {
                    at: cellPath,
                    children: true
                  });
        ...
        } else if (fragment.length === 1 && fragment[0].type === import_platejs42.KEYS.table) {
...
var withNormalizeTable = ({
  ...
  transforms: {
    normalizeNode([n, path]) {
      ...
      if (import_platejs43.ElementApi.isElement(n)) {
        if (n.type === type) {
          const node = n;
          if (!node.children.some(
            (child) => import_platejs43.ElementApi.isElement(child) && child.type === editor.getType(import_platejs43.KEYS.tr)
          )) {
            editor.tf.removeNodes({ at: path });
            return;
          }
        ...
        if (n.type === editor.getType(import_platejs43.KEYS.tr)) {
        ...
        if (getCellTypes(editor).includes(n.type)) {
          ...
          const { children } = node;
          const parentEntry = editor.api.parent(path);
          if (parentEntry?.[0].type !== editor.getType(import_platejs43.KEYS.tr)) {
            editor.tf.unwrapNodes({
              at: path
            });
            return;
          }
          if (import_platejs43.TextApi.isText(children[0])) {
    ...
    setFragmentData(data, originEvent) {
      ...
      editor.tf.withoutNormalizing(() => {
        tableRows.forEach((row) => {
          ...
          const cellStrings = [];
          const rowElement = row.type === editor.getType(import_platejs44.KEYS.th) ? document.createElement("th") : document.createElement("tr");
...
`;
        ...
        const _tableEntry = editor.api.node({
          at: tablePath,
          match: { type: import_platejs44.KEYS.table }
        });
        ...
        const apply = () => {
          ...
          matchesCell.forEach(([_cell, cellPath]) => {
            const textNodeEntry = editor.api.nodes({
              at: cellPath,
              match: (n) => import_platejs45.TextApi.isText(n)
            });
      ...
      setNodes(props, options) {
        const apply = () => {
          ...
          if (options?.at) {
            ...
            if (range && !import_platejs45.RangeApi.includes(selection, range)) {
              return;
            }
          }
          const matchesCell = getTableGridAbove(editor, { format: "cell" });
          ...
          setNodes(props, {
            ...options,
            match: (0, import_platejs45.combineTransformMatchOptions)(
              editor,
              (_, p) => matchesCell.some(
                ([_2, cellPath]) => import_platejs45.PathApi.isCommon(cellPath, p)
              ),
              options
            )
          });
...
var BaseTableRowPlugin = (0, import_platejs46.createSlatePlugin)({
  key: import_platejs46.KEYS.tr,
...
var BaseTableCellPlugin = (0, import_platejs46.createSlatePlugin)({
  key: import_platejs46.KEYS.td,
  node: {
...
var BaseTableCellHeaderPlugin = (0, import_platejs46.createSlatePlugin)({
  key: import_platejs46.KEYS.th,
  node: {
    dangerouslyAllowAttributes: ["colspan", "rowspan"],
    isContainer: true,
    isElement: true,
...
var BaseTablePlugin = (0, import_platejs46.createTSlatePlugin)({
  key: import_platejs46.KEYS.table,
  node: {
    isContainer: true,
    isElement: true
  },
  ...
  create: {
    table: (0, import_platejs46.bindFirst)(getEmptyTableNode, editor),
    tableCell: (0, import_platejs46.bindFirst)(getEmptyCellNode, editor),
    tableRow: (0, import_platejs46.bindFirst)(getEmptyRowNode, editor)
  },
  table: {
    getCellBorders: (0, import_platejs46.bindFirst)(getTableCellBorders, editor),
    getCellSize: (0, import_platejs46.bindFirst)(getTableCellSize, editor),
    getColSpan,
    getRowSpan,
  ...
  insert: {
    table: (0, import_platejs46.bindFirst)(insertTable, editor),
    tableColumn: (0, import_platejs46.bindFirst)(insertTableColumn, editor),
    tableRow: (0, import_platejs46.bindFirst)(insertTableRow, editor)
  },
  remove: {
    table: (0, import_platejs46.bindFirst)(deleteTable, editor),
    tableColumn: (0, import_platejs46.bindFirst)(deleteColumn, editor),
    tableRow: (0, import_platejs46.bindFirst)(deleteRow, editor)
  },
  table: {
    merge: (0, import_platejs46.bindFirst)(mergeTableCells, editor),
    split: (0, import_platejs46.bindFirst)(splitTableCell, editor)
  }
