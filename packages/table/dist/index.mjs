...
import { getEditorPlugin as getEditorPlugin5, KEYS as KEYS13 } from "platejs";
...
import { PathApi as PathApi3 } from "platejs";
...
var getTableAbove = (editor, options) => editor.api.block({
  above: true,
  match: {
    type: editor.getType(KEYS7.table)
  },
  ...options
});
...
import { getEditorPlugin as getEditorPlugin3, KEYS as KEYS8 } from "platejs";
...
var getTableEntries = (editor, { at = editor.selection } = {}) => {
  ...
  const rowEntry = editor.api.above({
    at: cellPath,
    match: { type: editor.getType(KEYS9.tr) }
  });
  if (!rowEntry) return;
  const [, rowPath] = rowEntry;
  const tableEntry = editor.api.above({
    at: rowPath,
    match: { type: editor.getType(KEYS9.table) }
  });
...
var getTableGridByRange = (editor, { at, format = "table" }) => {
  const { api } = editor.getPlugin({ key: KEYS10.table });
...
var getTableGridAbove = (editor, { format = "table", ...options } = {}) => {
  const { api } = editor.getPlugin({ key: KEYS11.table });
...
import { PathApi as PathApi8 } from "platejs";
...
var deleteTableMergeColumn = (editor) => {
  const type = editor.getType(KEYS13.table);
  const tableEntry = editor.api.above({
    match: { type }
  });
  if (!tableEntry) return;
...
import { getEditorPlugin as getEditorPlugin7, KEYS as KEYS14 } from "platejs";
...
var insertTableMergeColumn = (editor, {
  ...
  if (at && !fromCell) {
    ...
    if (table?.type === editor.getType(KEYS15.table)) {
...
var insertTableMergeRow = (editor, {
  ...
  if (at && !fromRow) {
    ...
    if (table?.type === editor.getType(KEYS16.table)) {
  ...
  const trEntry = editor.api.block({
    at: fromRow,
    match: { type: editor.getType(KEYS16.tr) }
  });
  if (!trEntry) return;
...
import { getEditorPlugin as getEditorPlugin10, KEYS as KEYS17 } from "platejs";
  ...
  editor.tf.withoutNormalizing(() => {
    ...
    const mergedCell = {
      ...api.create.tableCell({
        ...
        header: cellEntries[0][0].type === editor.getType(KEYS17.th)
      }),
      colSpan,
      rowSpan
    };
...
import { getEditorPlugin as getEditorPlugin11, KEYS as KEYS18 } from "platejs";
var splitTableCell = (editor) => {
  const { api } = getEditorPlugin11(editor, BaseTablePlugin);
  const tableRowType = editor.getType(KEYS18.tr);
  ...
  editor.tf.withoutNormalizing(() => {
    const createEmptyCell = (children) => {
      return {
        ...api.create.tableCell({
          children,
          header: cellElem.type === editor.getType(KEYS18.th)
        }),
        colSpan: 1,
        rowSpan: 1
      };
...
import { getEditorPlugin as getEditorPlugin12, KEYS as KEYS19 } from "platejs";
...
import { getEditorPlugin as getEditorPlugin13, KEYS as KEYS20 } from "platejs";
var deleteRow = (editor) => {
  ...
  if (editor.api.some({
    ...
    const currentRowItem = editor.api.above({
      match: { type: editor.getType(KEYS20.tr) }
    });
...
var deleteTable = (editor) => {
  if (editor.api.some({
    match: { type: editor.getType(KEYS21.table) }
  })) {
...
var insertTable = (editor, { colCount = 2, header, rowCount = 2 } = {}, { select: shouldSelect, ...options } = {}) => {
  const { api } = editor.getPlugin({ key: KEYS22.table });
  const type = editor.getType(KEYS22.table);
  editor.tf.withoutNormalizing(() => {
...
import { getEditorPlugin as getEditorPlugin14, KEYS as KEYS23, NodeApi as NodeApi5, PathApi as PathApi12 } from "platejs";
  ...
  const getEmptyRowNode2 = () => ({
    children: trNode.children.map((_, i) => {
      ...
      const isHeaderColumn = !hasSingleRow && tableEntry[0].children.every(
        (n) => n.children[i].type === editor.getType(KEYS24.th)
      );
...
import { NodeApi as NodeApi7 } from "platejs";
...
var withApplyTable = ({
  ...
  transforms: {
    apply(op) {
      ...
      const isTableOperation = (op.type === "remove_node" || op.type === "move_node") && opType && [editor.getType(KEYS28.tr), tableType, ...getCellTypes(editor)].includes(
        opType
      );
  ...
  transforms: {
    insertFragment(fragment) {
      ...
      if (insertedTable) {
        ...
        } else if (fragment.length === 1 && fragment[0].type === KEYS29.table) {
      ...
      if (ElementApi2.isElement(n)) {
        if (n.type === type) {
          ...
          if (!node.children.some(
            (child) => ElementApi2.isElement(child) && child.type === editor.getType(KEYS30.tr)
          )) {
            editor.tf.removeNodes({ at: path });
            return;
          }
        ...
        if (n.type === editor.getType(KEYS30.tr)) {
        ...
        if (getCellTypes(editor).includes(n.type)) {
          ...
          const { children } = node;
          const parentEntry = editor.api.parent(path);
          if (parentEntry?.[0].type !== editor.getType(KEYS30.tr)) {
            editor.tf.unwrapNodes({
              at: path
            });
            return;
...
import { KEYS as KEYS31 } from "platejs";
    ...
    setFragmentData(data, originEvent) {
      ...
      editor.tf.withoutNormalizing(() => {
        tableRows.forEach((row) => {
          ...
          const cellStrings = [];
          const rowElement = row.type === editor.getType(KEYS31.th) ? document.createElement("th") : document.createElement("tr");
      ...
      setNodes(props, options) {
        const apply = () => {
          ...
          if (options?.at) {
            ...
            if (range && !RangeApi3.includes(selection, range)) {
              return;
            }
          }
          const matchesCell = getTableGridAbove(editor, { format: "cell" });
          ...
          setNodes(props, {
            ...
            match: combineTransformMatchOptions(
              ...
              (_, p) => matchesCell.some(
                ([_2, cellPath]) => PathApi14.isCommon(cellPath, p)
              ),
              options
            )
          });
