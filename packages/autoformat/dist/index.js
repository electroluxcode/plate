  ...
  for (const match of matches) {
    ...
    if (triggerAtBlockStart) {
      ...
      const hasVoidNode = editor.api.some({
        at: matchRange,
        match: (n) => import_platejs.ElementApi.isElement(n) && editor.api.isVoid(n)
      });
...
// src/lib/AutoformatPlugin.ts
var AutoformatPlugin = (0, import_platejs2.createTSlatePlugin)({
  key: import_platejs2.KEYS.autoformat,
  editOnly: true,
  options: {
    rules: []
  }
      ...
      transforms: {
        deleteBackward(unit = "character") {
          const apply = () => {
            ...
            const [start, end] = import_platejs2.RangeApi.edges(selection);
