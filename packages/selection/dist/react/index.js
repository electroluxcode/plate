...
__export(react_exports, {
  ...
  addOnContextMenu: () => addOnContextMenu,
...
var BlockMenuPlugin = (0, import_react.createTPlatePlugin)({
  key: import_platejs.KEYS.blockMenu,
  editOnly: true,
  options: {
    openId: null,
    position: {
  ...
  ({ api, editor }) => ({
    showContextMenu: (blockId, position) => {
      editor.getApi({ key: import_platejs.KEYS.blockSelection }).blockSelection?.set(blockId);
...
var addOnContextMenu = (editor, {
  ...
  if (editor.selection?.focus && disabledWhenFocused) {
    ...
    if (nodeEntry && elementPath && import_platejs2.PathApi.isCommon(elementPath, nodeEntry[1])) {
  ...
  if (id) {
    if (event?.shiftKey) {
      ...
      if (!clickAlreadySelected) {
        editor.setOption(BlockSelectionPlugin, "selectedIds", /* @__PURE__ */ new Set([id]));
      }
    }
  }
};
var useBlockSelectable = () => {
  ...
  const { api, editor, getOption, getOptions, setOption } = (0, import_react3.useEditorPlugin)({
    key: import_platejs2.KEYS.blockSelection
  });
  return {
...
var CursorOverlayPlugin = (0, import_react8.createTPlatePlugin)({
  key: import_platejs3.KEYS.cursorOverlay,
  editOnly: {
    render: false
  },
  options: { cursors: {} }
  ...
  handlers: {
    ...
    onDragOver: ({ api, editor, event }) => {
      if (!editor.plugins.dnd || editor.getOptions({ key: import_platejs3.KEYS.dnd }).isDragging) {
        return;
      }
...
var getCaretPosition = (selectionRects, range) => {
  const isCollapsed = range && import_platejs4.RangeApi.isCollapsed(range);
  const isBackward = range && import_platejs4.RangeApi.isBackward(range);
...
var getSelectionRects = (editor, {
  ...
  const [start, end] = import_platejs5.RangeApi.edges(range);
  ...
  const textEntries = editor.api.nodes({
    at: range,
    match: import_platejs5.TextApi.isText
  });
  for (const [textNode, textPath] of textEntries) {
    ...
    const isStartNode = import_platejs5.PathApi.equals(textPath, start.path);
    const isEndNode = import_platejs5.PathApi.equals(textPath, end.path);
...
var useSelectionArea = () => {
  const { api, editor, getOption, getOptions, setOption } = (0, import_react16.useEditorPlugin)(BlockSelectionPlugin);
  const { areaOptions } = getOptions();
  const areaRef = import_react15.default.useRef({
    ids: /* @__PURE__ */ new Set()
  });
  const trsRef = import_react15.default.useRef({
    ids: /* @__PURE__ */ new Set()
  });
  const onStart = () => {
    ...
    const selection = new SelectionArea({
      ...
      const apply = () => {
        ...
        added.forEach((id) => {
          ...
          if (!block) return;
          if (block[0].type === import_platejs6.KEYS.table) return;
          if (block[1].length === 1) {
            next.add(id);
      ...
      const normalize = () => {
        ...
        const isTableElement = (element) => element.type === import_platejs6.KEYS.table || element.type === import_platejs6.KEYS.tr || element.type === import_platejs6.KEYS.th;
        const isTableRowElement = (element) => element.type === import_platejs6.KEYS.tr || element.type === import_platejs6.KEYS.th;
        ...
        if (isTableOnlySelection) {
          ids.some((id) => {
            const block = getBlockById(id);
            if (!block || block[0].type !== import_platejs6.KEYS.table) return false;
            next.delete(id);
...
var selectInsertedBlocks = (editor) => {
  const { setOption } = (0, import_platejs7.getEditorPlugin)(editor, BlockSelectionPlugin);
...
var pasteSelectedBlocks = (editor, e) => {
  const { api } = (0, import_platejs8.getEditorPlugin)(editor, BlockSelectionPlugin);
  const entries = api.blockSelection.getNodes();
  if (entries.length > 0) {
    const entry = entries.at(-1);
    const [node, path] = entry;
    if (!editor.api.isEmpty(node)) {
      const at = import_platejs8.PathApi.next(path);
...
var BlockSelectionAfterEditable = () => {
  ...
  const { api, getOption, getOptions, setOption } = (0, import_react18.useEditorPlugin)({ key: import_platejs9.KEYS.blockSelection });
  ...
  const removeSelectedBlocks = import_react17.default.useCallback(
    (options = {}) => {
      const entries = [
        ...editor.api.nodes({
          at: [],
      ...
      editor.tf.withoutNormalizing(() => {
        ...
        if (editor.children.length === 0) {
          ...
          const prevPath = import_platejs9.PathApi.previous(firstPath);
          if (prevPath) {
  ...
  const handleKeyDown = import_react17.default.useCallback(
    (e) => {
      ...
      if ((0, import_platejs9.isHotkey)("shift+up")(e)) {
        e.preventDefault();
        e.stopPropagation();
        api.blockSelection.shiftSelection("up");
        return;
      }
      if ((0, import_platejs9.isHotkey)("shift+down")(e)) {
        e.preventDefault();
        e.stopPropagation();
        api.blockSelection.shiftSelection("down");
        return;
      }
      if ((0, import_platejs9.isHotkey)("escape")(e)) {
        api.blockSelection.deselect();
        return;
      }
      if ((0, import_platejs9.isHotkey)("mod+z")(e)) {
        editor.undo();
        selectInsertedBlocks(editor);
        return;
      }
      if ((0, import_platejs9.isHotkey)("mod+a")(e)) {
        api.blockSelection.selectAll();
        return;
      }
      if ((0, import_platejs9.isHotkey)("mod+shift+z")(e)) {
        editor.redo();
        selectInsertedBlocks(editor);
        return;
      }
      if ((0, import_platejs9.isHotkey)("mod+d")(e)) {
        e.preventDefault();
        editor.getTransforms(BlockSelectionPlugin).blockSelection.duplicate();
        return;
      }
      if (!getOption("isSelectingSome")) return;
      if ((0, import_platejs9.isHotkey)("enter")(e)) {
      ...
      if ((0, import_platejs9.isHotkey)(["backspace", "delete"])(e) && !isReadonly) {
        e.preventDefault();
        removeSelectedBlocks({
          selectPrevious: (0, import_platejs9.isHotkey)("backspace")(e)
        });
        return;
      }
      if ((0, import_platejs9.isHotkey)("up")(e)) {
        e.preventDefault();
        e.stopPropagation();
        api.blockSelection.moveSelection("up");
        return;
      }
      if ((0, import_platejs9.isHotkey)("down")(e)) {
        e.preventDefault();
        e.stopPropagation();
        api.blockSelection.moveSelection("down");
        return;
      }
      if (!isReadonly && e.key.length === 1 && // Only handle single character keys
...
var setSelectedIds = (editor, {
  ...
  const { getOptions, setOption } = (0, import_platejs10.getEditorPlugin)(
    editor,
    { key: import_platejs10.KEYS.blockSelection }
  );
  if (ids) {
...
var addSelectedRow = (editor, id, options = {}) => {
  const { api, getOptions, setOption } = (0, import_platejs10.getEditorPlugin)(
    editor,
    { key: import_platejs10.KEYS.blockSelection }
  );
  ...
  if (direction === "down") {
    if (anchorIsTop) {
      const belowEntry = editor.api.next({
        at: bottomPath,
        mode: "highest",
        match: (n, p) => api.blockSelection.isSelectable(n, p) && !import_platejs11.PathApi.isAncestor(p, bottomPath)
      });
    ...
    if (anchorIsBottom) {
      ...
      if (import_platejs11.PathApi.isAncestor(abovePath, topPath)) {
        newSelected.forEach((id) => {
          const entry = editor.api.node({ id, at: abovePath });
          if (!entry) return;
          if (import_platejs11.PathApi.isDescendant(entry[1], abovePath)) {
...
var duplicateBlockSelectionNodes = (editor) => {
  ...
  const path = import_platejs12.PathApi.next(lastBlock[1]);
...
var insertBlocksAndSelect = (editor, nodes, { at }) => {
  ...
  const insertedNodes = [import_platejs13.NodeApi.get(editor, at)];
  let count = 1;
  while (count < nodes.length) {
    at = import_platejs13.PathApi.next(at);
    const nextNode = import_platejs13.NodeApi.get(editor, at);
...
var BlockSelectionPlugin = (0, import_react21.createTPlatePlugin)({
  key: import_platejs14.KEYS.blockSelection,
  editOnly: true,
  handlers: {
  ...
  ({ api, editor, getOption, getOptions, setOption }) => ({
    addOnContextMenu: (0, import_platejs14.bindFirst)(addOnContextMenu, editor),
    moveSelection: (0, import_platejs14.bindFirst)(moveSelection, editor),
    setSelectedIds: (0, import_platejs14.bindFirst)(setSelectedIds, editor),
    shiftSelection: (0, import_platejs14.bindFirst)(shiftSelection, editor),
    add: (id) => {
      const next = new Set(getOptions().selectedIds);
      if (Array.isArray(id)) {
    ...
    getNodes: (options) => {
      const selectedIds = getOption("selectedIds");
      let nodes = [];
      nodes = editor.api.blocks({
        at: [],
        match: (n) => !!n.id && selectedIds?.has(n.id)
      });
      if (options?.sort) {
        nodes.sort(([, pathA], [, pathB]) => {
          return import_platejs14.PathApi.compare(pathA, pathB);
        });
      }
      if (options?.collapseTableRows) {
        const collapsedNodes = [];
        nodes.forEach(([node, path]) => {
          if (node.type === import_platejs14.KEYS.tr) {
            const tablePath = import_platejs14.PathApi.parent(path);
            const tableNodeEntry = editor.api.node(tablePath);
            const existingTableIndex = collapsedNodes.findIndex(
              ([existingNode]) => existingNode.type === tableNodeEntry[0].type && existingNode.id === tableNodeEntry[0].id
            );
  ...
  ({ api, editor, setOption }) => ({
    addSelectedRow: (0, import_platejs14.bindFirst)(addSelectedRow, editor),
    selectAll: () => {
  ...
  duplicate: (0, import_platejs14.bindFirst)(duplicateBlockSelectionNodes, editor),
  /** Insert blocks and select */
  insertBlocksAndSelect: (0, import_platejs14.bindFirst)(insertBlocksAndSelect, editor),
  /** Remove selected blocks */
  removeNodes: (0, import_platejs14.bindFirst)(removeBlockSelectionNodes, editor),
  /** Set selection based on block selection */
  select: (0, import_platejs14.bindFirst)(selectBlockSelectionNodes, editor),
  /**
   ...
   */
  selectBlocks: (0, import_platejs14.bindFirst)(selectBlocks, editor),
  /** Set block indent */
  setIndent: (0, import_platejs14.bindFirst)(setBlockSelectionIndent, editor),
  /** Set nodes on selected blocks */
  setNodes: (0, import_platejs14.bindFirst)(setBlockSelectionNodes, editor),
  /** Set texts on selected blocks */
  setTexts: (0, import_platejs14.bindFirst)(setBlockSelectionTexts, editor)
})).overrideEditor((ctx) => {
  const {
    api,
    api: { nodes },
...
var withBlockSelection = ({ api, editor, getOption }, callback) => {
  if (getOption("isSelectingSome")) {
    editor.tf.withoutNormalizing(() => {
      const blocks = editor.getApi(BlockSelectionPlugin).blockSelection.getNodes();
