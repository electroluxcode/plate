...
var useSelectionArea = () => {
  const { api, editor, getOption, getOptions, setOption } = (0, import_react5.useEditorPlugin)(BlockSelectionPlugin);
  const { areaOptions } = getOptions();
  const areaRef = import_react4.default.useRef({
    ids: /* @__PURE__ */ new Set()
  });
  const trsRef = import_react4.default.useRef({
    ids: /* @__PURE__ */ new Set()
  });
  const onStart = () => {
    ...
    const selection = new SelectionArea({
      ...
      const apply = () => {
        ...
        added.forEach((id) => {
          ...
          if (!block) return;
          if (block[0].type === import_platejs3.KEYS.table) return;
          if (block[1].length === 1) {
            next.add(id);
      ...
      const normalize = () => {
        ...
        const isTableElement = (element) => element.type === import_platejs3.KEYS.table || element.type === import_platejs3.KEYS.tr || element.type === import_platejs3.KEYS.th;
        const isTableRowElement = (element) => element.type === import_platejs3.KEYS.tr || element.type === import_platejs3.KEYS.th;
        ...
        if (isTableOnlySelection) {
          ids.some((id) => {
            const block = getBlockById(id);
            if (!block || block[0].type !== import_platejs3.KEYS.table) return false;
            next.delete(id);
...
var selectInsertedBlocks = (editor) => {
  const { setOption } = (0, import_platejs4.getEditorPlugin)(editor, BlockSelectionPlugin);
...
var pasteSelectedBlocks = (editor, e) => {
  const { api } = (0, import_platejs5.getEditorPlugin)(editor, BlockSelectionPlugin);
  const entries = api.blockSelection.getNodes();
  if (entries.length > 0) {
    const entry = entries.at(-1);
    const [node, path] = entry;
    if (!editor.api.isEmpty(node)) {
      const at = import_platejs5.PathApi.next(path);
...
var BlockSelectionAfterEditable = () => {
  ...
  const { api, getOption, getOptions, setOption } = (0, import_react7.useEditorPlugin)({ key: import_platejs6.KEYS.blockSelection });
  ...
  const removeSelectedBlocks = import_react6.default.useCallback(
    (options = {}) => {
      const entries = [
        ...editor.api.nodes({
          at: [],
      ...
      editor.tf.withoutNormalizing(() => {
        ...
        if (editor.children.length === 0) {
          ...
          const prevPath = import_platejs6.PathApi.previous(firstPath);
          if (prevPath) {
  ...
  const handleKeyDown = import_react6.default.useCallback(
    (e) => {
      ...
      if ((0, import_platejs6.isHotkey)("shift+up")(e)) {
        e.preventDefault();
        e.stopPropagation();
        api.blockSelection.shiftSelection("up");
        return;
      }
      if ((0, import_platejs6.isHotkey)("shift+down")(e)) {
        e.preventDefault();
        e.stopPropagation();
        api.blockSelection.shiftSelection("down");
        return;
      }
      if ((0, import_platejs6.isHotkey)("escape")(e)) {
        api.blockSelection.deselect();
        return;
      }
      if ((0, import_platejs6.isHotkey)("mod+z")(e)) {
        editor.undo();
        selectInsertedBlocks(editor);
        return;
      }
      if ((0, import_platejs6.isHotkey)("mod+a")(e)) {
        api.blockSelection.selectAll();
        return;
      }
      if ((0, import_platejs6.isHotkey)("mod+shift+z")(e)) {
        editor.redo();
        selectInsertedBlocks(editor);
        return;
      }
      if ((0, import_platejs6.isHotkey)("mod+d")(e)) {
        e.preventDefault();
        editor.getTransforms(BlockSelectionPlugin).blockSelection.duplicate();
        return;
      }
      if (!getOption("isSelectingSome")) return;
      if ((0, import_platejs6.isHotkey)("enter")(e)) {
      ...
      if ((0, import_platejs6.isHotkey)(["backspace", "delete"])(e) && !isReadonly) {
        e.preventDefault();
        removeSelectedBlocks({
          selectPrevious: (0, import_platejs6.isHotkey)("backspace")(e)
        });
        return;
      }
      if ((0, import_platejs6.isHotkey)("up")(e)) {
        e.preventDefault();
        e.stopPropagation();
        api.blockSelection.moveSelection("up");
        return;
      }
      if ((0, import_platejs6.isHotkey)("down")(e)) {
        e.preventDefault();
        e.stopPropagation();
...
var setSelectedIds = (editor, {
  ...
  const { getOptions, setOption } = (0, import_platejs7.getEditorPlugin)(
    editor,
    { key: import_platejs7.KEYS.blockSelection }
  );
  if (ids) {
...
var addSelectedRow = (editor, id, options = {}) => {
  const { api, getOptions, setOption } = (0, import_platejs7.getEditorPlugin)(
    editor,
    { key: import_platejs7.KEYS.blockSelection }
  );
  ...
  if (direction === "down") {
    if (anchorIsTop) {
      const belowEntry = editor.api.next({
        at: bottomPath,
        mode: "highest",
        match: (n, p) => api.blockSelection.isSelectable(n, p) && !import_platejs8.PathApi.isAncestor(p, bottomPath)
      });
    ...
    if (anchorIsBottom) {
      ...
      if (import_platejs8.PathApi.isAncestor(abovePath, topPath)) {
        newSelected.forEach((id) => {
          const entry = editor.api.node({ id, at: abovePath });
          if (!entry) return;
          if (import_platejs8.PathApi.isDescendant(entry[1], abovePath)) {
...
var duplicateBlockSelectionNodes = (editor) => {
  ...
  const path = import_platejs9.PathApi.next(lastBlock[1]);
...
var insertBlocksAndSelect = (editor, nodes, { at }) => {
  ...
  const insertedNodes = [import_platejs10.NodeApi.get(editor, at)];
  let count = 1;
  while (count < nodes.length) {
    at = import_platejs10.PathApi.next(at);
    const nextNode = import_platejs10.NodeApi.get(editor, at);
...
var BlockSelectionPlugin = (0, import_react10.createTPlatePlugin)({
  key: import_platejs11.KEYS.blockSelection,
  editOnly: true,
  handlers: {
  ...
  ({ api, editor, getOption, getOptions, setOption }) => ({
    addOnContextMenu: (0, import_platejs11.bindFirst)(addOnContextMenu, editor),
    moveSelection: (0, import_platejs11.bindFirst)(moveSelection, editor),
    setSelectedIds: (0, import_platejs11.bindFirst)(setSelectedIds, editor),
    shiftSelection: (0, import_platejs11.bindFirst)(shiftSelection, editor),
    add: (id) => {
      const next = new Set(getOptions().selectedIds);
      if (Array.isArray(id)) {
    ...
    getNodes: (options) => {
      ...
      if (options?.sort) {
        nodes.sort(([, pathA], [, pathB]) => {
          return import_platejs11.PathApi.compare(pathA, pathB);
        });
      }
      if (options?.collapseTableRows) {
        const collapsedNodes = [];
        nodes.forEach(([node, path]) => {
          if (node.type === import_platejs11.KEYS.tr) {
            const tablePath = import_platejs11.PathApi.parent(path);
  ...
  ({ api, editor, setOption }) => ({
    addSelectedRow: (0, import_platejs11.bindFirst)(addSelectedRow, editor),
    selectAll: () => {
  ...
  duplicate: (0, import_platejs11.bindFirst)(duplicateBlockSelectionNodes, editor),
  /** Insert blocks and select */
  insertBlocksAndSelect: (0, import_platejs11.bindFirst)(insertBlocksAndSelect, editor),
  /** Remove selected blocks */
  removeNodes: (0, import_platejs11.bindFirst)(removeBlockSelectionNodes, editor),
  /** Set selection based on block selection */
  select: (0, import_platejs11.bindFirst)(selectBlockSelectionNodes, editor),
  /**
   ...
   */
  selectBlocks: (0, import_platejs11.bindFirst)(selectBlocks, editor),
  /** Set block indent */
  setIndent: (0, import_platejs11.bindFirst)(setBlockSelectionIndent, editor),
  /** Set nodes on selected blocks */
  setNodes: (0, import_platejs11.bindFirst)(setBlockSelectionNodes, editor),
  /** Set texts on selected blocks */
  setTexts: (0, import_platejs11.bindFirst)(setBlockSelectionTexts, editor)
})).overrideEditor((ctx) => {
  const {
    api,
    api: { nodes },
