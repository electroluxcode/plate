...
import { bindFirst, KEYS as KEYS6, PathApi as PathApi7 } from "platejs";
...
import { isHotkey, KEYS as KEYS4, PathApi as PathApi3 } from "platejs";
import {
    ...
    const selection = new SelectionArea({
      ...
      const apply = () => {
        ...
        added.forEach((id) => {
          ...
          if (!block) return;
          if (block[0].type === KEYS3.table) return;
          if (block[1].length === 1) {
            next.add(id);
      ...
      const normalize = () => {
        ...
        const isTableElement = (element) => element.type === KEYS3.table || element.type === KEYS3.tr || element.type === KEYS3.th;
        const isTableRowElement = (element) => element.type === KEYS3.tr || element.type === KEYS3.th;
        ...
        if (isTableOnlySelection) {
          ids.some((id) => {
            const block = getBlockById(id);
            if (!block || block[0].type !== KEYS3.table) return false;
            next.delete(id);
...
import { getEditorPlugin as getEditorPlugin2, PathApi as PathApi2 } from "platejs";
...
var BlockSelectionAfterEditable = () => {
  ...
  const { api, getOption, getOptions, setOption } = useEditorPlugin3({ key: KEYS4.blockSelection });
  const isSelectingSome = usePluginOption(
    BlockSelectionPlugin,
    "isSelectingSome"
  );
  const selectedIds = usePluginOption(BlockSelectionPlugin, "selectedIds");
  const removeSelectedBlocks = React2.useCallback(
    (options = {}) => {
      const entries = [
        ...editor.api.nodes({
          at: [],
      ...
      editor.tf.withoutNormalizing(() => {
        ...
        if (editor.children.length === 0) {
          ...
          const prevPath = PathApi3.previous(firstPath);
          if (prevPath) {
  ...
  const handleCut = React2.useCallback(
    (e) => {
      e.preventDefault();
      if (getOption("isSelectingSome")) {
        copySelectedBlocks(editor);
...
import { getEditorPlugin as getEditorPlugin4, KEYS as KEYS5 } from "platejs";
...
import { NodeApi, PathApi as PathApi6 } from "platejs";
    ...
    getNodes: (options) => {
      ...
      if (options?.sort) {
        nodes.sort(([, pathA], [, pathB]) => {
          return PathApi7.compare(pathA, pathB);
        });
      }
      if (options?.collapseTableRows) {
        const collapsedNodes = [];
        nodes.forEach(([node, path]) => {
          if (node.type === KEYS6.tr) {
            const tablePath = PathApi7.parent(path);
