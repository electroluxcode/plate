...
- **实时协作**：基于 [ShareDB](https://github.com/share/sharedb) 的 OT（Operational Transformation）实时协作
- **自动重连**：内置断线重连机制
...
// 分别初始化不同的文档
useEffect(() => {
  editor1.api.ot.init({ id: 'doc-1' });
  editor2.api.ot.init({ id: 'doc-2' });
}, []);
```
...
- [ottype-slate-test](https://www.npmjs.com/package/ottype-slate-test) - Slate OT 类型定义
