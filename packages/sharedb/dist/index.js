...
__export(index_exports, {
  BaseOtPlugin: () => BaseOtPlugin,
  OperationQueue: () => OperationQueue,
  ShareDBPlugin: () => ShareDBPlugin,
  createShareDBClient: () => createShareDBClient,
  withPlateOt: () => withPlateOt
});
...
// src/lib/BaseOtPlugin.ts
var BaseOtPlugin = (0, import_platejs.createTSlatePlugin)({
  key: "sharedb",
  priority: 1e3,
  ...
  applyRemoteOperationWithoutEditor: (operations) => {
    ...
    const updateOp = () => {
      ...
      opsQueue.forEach((op) => {
        import_slate.Transforms.transform(editor, op);
      });
      editor.onChange();
    };
    updateOp();
...
// src/utils/createShareDBClient.ts
async function createShareDBClient(config) {
  const [{ default: ShareDB }, { default: ReconnectingWebSocket }] = await Promise.all([
  ...
  const socket = new ReconnectingWebSocket(config.url, [], {
    ...
    minReconnectionDelay: config.reconnection?.interval || 3e3
  });
  const connection = new ShareDB.Connection(socket);
...
var ShareDBPlugin = (0, import_react.toPlatePlugin)(BaseOtPlugin);
