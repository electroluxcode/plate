...
var withComment = ({
  ...
  transforms: {
    normalizeNode(entry) {
      ...
      if (node[KEYS5.comment] && !node[getDraftCommentKey()] && getCommentCount(node) < 1) {
        editor.tf.unsetNodes(KEYS5.comment, { at: path });
        return;
      }
  ...
  ({ api, editor, tf, type }) => ({
    removeMark: () => {
      ...
      editor.tf.withoutNormalizing(() => {
        ...
        editor.tf.removeMark(KEYS6.comment);
      });
    },
    setDraft: (options = {}) => {
      tf.setNodes(
    ...
    unsetMark: (options) => {
      ...
      nodes.forEach(([node]) => {
        ...
        if (isOverlapping) {
          ...
          unsetKeys = [KEYS6.comment, getDraftCommentKey(), getCommentKey(id)];
        }
...
// src/react/CommentPlugin.tsx
var CommentPlugin = toPlatePlugin(BaseCommentPlugin);
