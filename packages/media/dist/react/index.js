...
var withImageUpload = ({
  ...
  transforms: {
    insertData(dataTransfer) {
      ...
      const text2 = dataTransfer.getData(mimeType);
...
var ImagePreviewStore = (0, import_platejs14.createZustandStore)(
  {
    boundingClientRect: {},
    currentPreview: null,
...
var getUrlList = (editor) => {
  const enties = editor.api.nodes({
    at: [],
    match: (n) => n.type === import_platejs15.KEYS.img
  });
...
var useImagePreview = ({ scrollSpeed }) => {
  ...
  (0, import_react3.useEffect)(() => {
    const keydown = (e) => {
      if ((0, import_platejs16.isHotkey)("escape")(e)) {
        e.stopPropagation();
        onClose();
      }
    };
...
var useScaleInput = () => {
  ...
  return {
    props: {
      ...
      onKeyDown: (e) => {
        if ((0, import_platejs17.isHotkey)("enter")(e)) {
          if (Number(value) <= 50) {
...
var useMediaState = ({
  ...
  const embed = import_react11.default.useMemo(() => {
    if (!urlParsers || type !== editor.getType(import_platejs18.KEYS.video) && type !== editor.getType(import_platejs18.KEYS.mediaEmbed))
      return;
...
var FloatingMediaStore = (0, import_platejs19.createZustandStore)(
  {
    isEditing: false,
    url: ""
  },
...
var submitFloatingMedia = (editor, {
  ...
  const { isUrl: _isUrl = import_platejs20.isUrl, transformUrl } = editor.getOptions(plugin);
...
var updateUploadHistory = (editor, node) => {
  const index = editor.history.undos.findLastIndex(
    (batch2) => batch2[import_platejs21.KEYS.placeholder] && batch2.operations.some(
...
var insertMedia2 = (editor, files, options) => {
  ...
  Array.from(files).forEach((file, index) => {
    if (index === 0) {
      ...
      currentAt = currentAt ? import_platejs22.PathApi.next(currentAt) : void 0;
    }
    const id = (0, import_platejs22.nanoid)();
      ...
      editor.tf.insertNodes(
        {
          ...
          type: editor.getType(import_platejs22.KEYS.placeholder)
        },
...
var PlaceholderPlugin = (0, import_react18.toTPlatePlugin)(BasePlaceholderPlugin, {
  options: {
    ...
    uploadConfig: {
      audio: {
        maxFileCount: 1,
        maxFileSize: "8MB",
        mediaType: import_platejs23.KEYS.audio,
        minFileCount: 1
      },
      blob: {
        maxFileCount: 1,
        maxFileSize: "8MB",
        mediaType: import_platejs23.KEYS.file,
        minFileCount: 1
      },
      image: {
        maxFileCount: 3,
        maxFileSize: "4MB",
        mediaType: import_platejs23.KEYS.img,
        minFileCount: 1
      },
      pdf: {
        maxFileCount: 1,
      ...
      video: {
        maxFileCount: 1,
        maxFileSize: "16MB",
        mediaType: import_platejs23.KEYS.video,
        minFileCount: 1
      }
    },
    uploadingFiles: {}
  ...
  transforms: {
    writeHistory(stack, batch) {
      if (isHistoryMarking(editor)) {
        const newBatch = {
          ...batch,
          [import_platejs23.KEYS.placeholder]: true
        };
  ...
  insert: {
    media: (0, import_platejs23.bindFirst)(insertMedia2, editor)
  }
    ...
    onPaste: ({ editor, event, tf }) => {
      ...
      if (files.length > 0 && !types2.includes(TEXT_HTML)) {
        ...
        if (ancestor) {
          ...
          if (import_platejs23.NodeApi.string(node).length === 0) {
...
var usePlaceholderElementState = () => {
  ...
  const { mediaType } = (0, import_react20.useElement)(import_platejs24.KEYS.placeholder);
...
var usePlaceholderPopoverState = () => {
  ...
  const element = (0, import_react21.useElement)(import_platejs25.KEYS.placeholder);
