...
import { createPrimitiveComponent as createPrimitiveComponent2 } from "platejs/react";
var usePreviewImage = () => {
  const currentPreview = useImagePreviewValue("currentPreview");
  const translate = useImagePreviewValue("translate");
  const scale = useImagePreviewValue("scale");
...
import { isHotkey as isHotkey2 } from "platejs";
var useScaleInput = () => {
  const scale = useImagePreviewValue("scale");
...
var useMediaState = ({
  ...
  const embed = React3.useMemo(() => {
    if (!urlParsers || type !== editor.getType(KEYS12.video) && type !== editor.getType(KEYS12.mediaEmbed))
      return;
...
import { useEditorRef as useEditorRef4 } from "platejs/react";
...
import { createPrimitiveComponent as createPrimitiveComponent3, useElement as useElement3 } from "platejs/react";

// src/react/media/FloatingMedia/FloatingMediaStore.ts
import { createZustandStore as createZustandStore2 } from "platejs";
...
var updateUploadHistory = (editor, node) => {
  const index = editor.history.undos.findLastIndex(
    (batch2) => batch2[KEYS13.placeholder] && batch2.operations.some(
      ...
      editor.tf.insertNodes(
        {
          ...
          type: editor.getType(KEYS14.placeholder)
        },
  ...
  options: {
    ...
    uploadConfig: {
      audio: {
        maxFileCount: 1,
        maxFileSize: "8MB",
        mediaType: KEYS15.audio,
        minFileCount: 1
      },
      blob: {
        maxFileCount: 1,
        maxFileSize: "8MB",
        mediaType: KEYS15.file,
        minFileCount: 1
      },
      image: {
        maxFileCount: 3,
        maxFileSize: "4MB",
        mediaType: KEYS15.img,
        minFileCount: 1
      },
      pdf: {
        maxFileCount: 1,
...
import { createAtomStore as createAtomStore2 } from "platejs/react";
var {
  PlaceholderProvider,
  placeholderStore,
  usePlaceholderSet,
...
var usePlaceholderElementState = () => {
  ...
  const { mediaType } = useElement5(KEYS16.placeholder);
...
var usePlaceholderPopoverState = () => {
  ...
  const element = useElement6(KEYS17.placeholder);
