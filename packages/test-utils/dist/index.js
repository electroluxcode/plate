...
var resolveDescendants = (children) => {
  ...
  const addChild = (child) => {
    ...
    if (import_slate.TextApi.isText(child)) {
      const c = child;
      if (import_slate.TextApi.isText(prev) && STRINGS.has(prev) && STRINGS.has(c) && import_slate.TextApi.equals(prev, c, { loose: true })) {
        prev.text += c.text;
      } else {
        nodes.push(c);
      }
    } else if (import_slate.ElementApi.isElement(child)) {
      nodes.push(child);
    } else if (child instanceof Token) {
      let n = nodes.at(-1);
      if (!import_slate.TextApi.isText(n)) {
        addChild("");
        n = nodes.at(-1);
      }
...
var createText = (tagName, attributes, children) => {
  ...
  if (!import_slate.TextApi.isText(node)) {
    throw new Error(`
...
var createEditor = (makeEditor2) => (tagName, attributes, children) => {
  ...
  for (const child of children) {
    if (import_slate.RangeApi.isRange(child)) {
  ...
  for (const [node, path] of import_slate.NodeApi.texts(editor)) {
  ...
  } else if (import_slate.RangeApi.isRange(selection)) {
...
var DEFAULT_CREATORS = {
  anchor: createAnchor,
  cursor: createCursor,
  editor: createEditor(import_slate2.createEditor),
...
var jsx = createHyperscript({
  creators: {
    htext: import_slate_hyperscript.createText
  },
  elements
});
var jsxt = (0, import_slate_hyperscript.createHyperscript)({
  creators: {
    htext: import_slate_hyperscript.createText
  },
  elements
