...
var without = (x, { ignoreDeep = [], ignoreShallow = [] } = {}) => {
  ...
  if (!(0, import_isPlainObject.default)(x)) return x;
  const obj = x;
  const result = {};
...
var isEqual = (value, other, options) => (0, import_isEqual.default)(without(value, options), without(other, options));
...
var InlineNodeCharMap = class {
  ...
  replaceCharWithNode(haystack, needle, replacementNode) {
    return haystack.flatMap((haystackNode) => {
      if (!import_platejs.TextApi.isText(haystackNode)) return [haystackNode];
      const splitText = haystackNode.text.split(needle);
      if (splitText.length === 1) return [haystackNode];
      const replacementWithProps = {
        ...replacementNode,
        ...import_platejs.NodeApi.extractProps(haystackNode)
      };
      ...
      return nodeList.filter((n) => !import_platejs.TextApi.isText(n) || n.text.length > 0);
    });
  }
  // Replace non-text nodes with a text node containing a unique char
  nodeToText(node) {
    if (import_platejs.TextApi.isText(node)) return node;
...
var applyInsertText = (editor, apply, op) => {
  const node = import_platejs2.NodeApi.get(editor, op.path);
  apply(op);
...
var applyMergeNode = (editor, apply, op) => {
  const oldNode = import_platejs2.NodeApi.get(editor, op.path);
  const properties = import_platejs2.NodeApi.extractProps(oldNode);
  const prevNodePath = import_platejs2.PathApi.previous(op.path);
  const prevNode = import_platejs2.NodeApi.get(editor, prevNodePath);
  const newProperties = import_platejs2.NodeApi.extractProps(prevNode);
  apply(op);
...
var applySplitNode = (editor, apply, op) => {
  ...
  apply(op);
  const newNodePath = import_platejs2.PathApi.next(op.path);
...
var commitChangesToDiffs = (editor, { getDeleteProps, getInsertProps, getUpdateProps }) => {
  withoutRecordingOperations(editor, () => {
    ...
    flatUpdates.forEach(({ newProperties, properties, range }) => {
      const node = import_platejs2.NodeApi.get(editor, range.anchor.path);
...
var flattenPropsChanges = (editor) => {
  ...
  const rangePoints = (0, import_uniqWith.default)(
    unsortedRangePoints.sort(import_platejs2.PointApi.compare),
    import_platejs2.PointApi.equals
  );
  ...
  const flatUpdates = flatRanges.map((flatRange) => {
    const getIntersectingChanges = (changes) => changes.filter(({ rangeRef }) => {
      ...
      const intersection = import_platejs2.RangeApi.intersection(range, flatRange);
      if (!intersection) return false;
      return import_platejs2.RangeApi.isExpanded(intersection);
    });
    ...
    if ((0, import_isEqual2.default)(initialProps, finalProps)) return null;
    const properties = {};
    const newProperties = {};
...
function transformDiffTexts(nodes, nextNodes, options) {
  ...
  const charGenerator = unusedCharGenerator({
    ...
    skipChars: nodes.concat(nextNodes).filter(import_platejs3.TextApi.isText).map((n) => n.text).join("")
  });
  ...
  const nodesEditor = withChangeTracking((0, import_platejs3.createEditor)(), options);
  nodesEditor.children = [{ children: texts, type: import_platejs3.KEYS.p }];
...
function splitTextNodes(node, split, options) {
  ...
  for (let i = 0; i < split.length - 1; i++) {
    ...
    splitPath = import_platejs3.PathApi.next(splitPath);
...
function diffNodes(originNodes, targetNodes, { elementsAreRelated, ignoreProps }) {
  ...
  originNodes.forEach((originNode) => {
    ...
    relatedNode = remainingTargetNodes.find((targetNode) => {
      if (import_platejs4.ElementApi.isElement(originNode) && import_platejs4.ElementApi.isElement(targetNode)) {
...
function isEqualNode(value, other, ignoreProps) {
  return import_platejs4.ElementApi.isElement(value) && import_platejs4.ElementApi.isElement(other) && value.children !== null && other.children !== null && isEqual(value, other, {
    ignoreDeep: ignoreProps,
    ignoreShallow: ["children"]
  });
}
function isEqualNodeChildren(value, other) {
  if (import_platejs4.ElementApi.isElement(value) && import_platejs4.ElementApi.isElement(other) && isEqual(value.children, other.children)) {
    return true;
  }
  return import_platejs4.TextApi.isText(value) && import_platejs4.TextApi.isText(other) && isEqual(value.text, other.text);
}
...
function transformDiffDescendants(diff, { stringCharMapping, ...options }) {
  ...
  const isInlineList = (nodes) => nodes.every((node) => import_platejs5.TextApi.isText(node) || isInline(node));
  while (i < diff.length) {
    const chunk = diff[i];
    const op = chunk[0];
    const val = chunk[1];
...
var StringCharMapping = class {
  ...
  nodeToChar(node) {
    for (const [n, c2] of this._mappedNodes) {
      if ((0, import_isEqual3.default)(n, node)) {
        return c2;
      }
    }
    const c = this._charGenerator.next().value;
