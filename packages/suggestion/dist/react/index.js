...
var findSuggestionProps = (editor, { at, type }) => {
      ...
      if (!entry) {
        ...
        if (!entry && editor.api.isStart(start, at)) {
          ...
          if (lineBreakData?.isLineBreak) {
            return {
              id: lineBreakData?.id ?? (0, import_platejs3.nanoid)(),
  ...
  if (entry && getInlineSuggestionData(entry[0])?.type === type && isCurrentUserSuggestion(editor, entry[0])) {
    return {
      id: api.suggestion.nodeId(entry[0]) ?? (0, import_platejs3.nanoid)(),
...
var setSuggestionNodes = (editor, options) => {
  const at = (0, import_platejs4.getAt)(editor, options?.at) ?? editor.selection;
  if (!at) return;
  const { suggestionId = (0, import_platejs4.nanoid)() } = options ?? {};
  const _nodeEntries = editor.api.nodes({
    match: (n) => import_platejs4.ElementApi.isElement(n) && editor.api.isInline(n),
    ...options
  });
  ...
  editor.tf.withoutNormalizing(() => {
    ...
    const props = {
      ...
      [import_platejs4.KEYS.suggestion]: true
    };
  ...
  editor.tf.withoutNormalizing(() => {
    ...
    while (true) {
      ...
      const entryBlock = editor.api.node({
        at: pointCurrent,
        block: true,
        match: (n) => n[import_platejs5.KEYS.suggestion] && import_platejs5.TextApi.isText(n) && getInlineSuggestionData(n)?.type === "insert" && isCurrentUserSuggestion(editor, n)
      });
      ...
      if (editor.api.isAt({ at: range, blocks: true })) {
        ...
        if (previousAboveNode && import_platejs5.ElementApi.isElement(previousAboveNode[0])) {
          ...
          if (isBlockSuggestion) {
            ...
            if (node.suggestion.type === "insert") {
              editor.getApi(BaseSuggestionPlugin).suggestion.withoutSuggestions(() => {
                editor.tf.unsetNodes([import_platejs5.KEYS.suggestion], {
                  at: previousAboveNode[1]
                });
                editor.tf.mergeNodes({
                  at: import_platejs5.PathApi.next(previousAboveNode[1])
                });
              });
            }
          ...
          if (!isBlockSuggestion) {
            editor.tf.setNodes(
              {
                [import_platejs5.KEYS.suggestion]: {
                  id,
                  createdAt,
                  type: "remove",
                  userId: editor.getOptions(BaseSuggestionPlugin).currentUserId
      ...
      if (import_platejs5.PointApi.equals(pointCurrent, editor.selection.anchor)) {
        editor.tf.move({
          reverse,
          unit: "character"
        });
      ...
      const entryText = editor.getApi(BaseSuggestionPlugin).suggestion.node({
        at: range,
        isText: true,
        match: (n) => import_platejs5.TextApi.isText(n) && getInlineSuggestionData(n)?.type === "insert" && isCurrentUserSuggestion(editor, n)
      });
      if (entryText) {
...
var insertFragmentSuggestion = (editor, fragment, {
  ...
  editor.tf.withoutNormalizing(() => {
    ...
    fragment.forEach((n) => {
      if (import_platejs6.TextApi.isText(n)) {
        if (!n[import_platejs6.KEYS.suggestion]) {
          n[import_platejs6.KEYS.suggestion] = true;
        }
        ...
        n[import_platejs6.KEYS.suggestion] = {
          id,
          createdAt,
          type: "insert",
          userId: editor.getOptions(BaseSuggestionPlugin).currentUserId
...
var removeMarkSuggestion = (editor, key) => {
  editor.getApi(BaseSuggestionPlugin).suggestion.withoutSuggestions(() => {
    ...
    const match = (n) => {
      if (!import_platejs7.TextApi.isText(n)) return false;
      if (n[import_platejs7.KEYS.suggestion]) {
...
var addMarkSuggestion = (editor, key, value) => {
  editor.getApi(BaseSuggestionPlugin).suggestion.withoutSuggestions(() => {
    ...
    const match = (n) => {
      if (!import_platejs9.TextApi.isText(n)) return false;
    ...
    editor.tf.setNodes(
      {
        ...
        [import_platejs9.KEYS.suggestion]: true
      },
      {
        match,
        split: true
  ...
  transforms: {
    ...
    deleteBackward(unit) {
      ...
      if (getOptions().isSuggesting) {
        ...
        if (node?.[0][import_platejs10.KEYS.suggestion] && !node?.[0].suggestion.isLineBreak) {
        ...
        if (pointTarget) {
          ...
          if (isCrossBlock) {
            editor.tf.unsetNodes([import_platejs10.KEYS.suggestion], {
              at: pointTarget
            });
          }
        }
    ...
    insertBreak() {
      if (getOptions().isSuggesting) {
        ...
        if (path.length > 1 || node.type !== editor.getType(import_platejs10.KEYS.p)) {
        ...
        editor.tf.withoutMerging(() => {
          editor.tf.setNodes(
            {
              [import_platejs10.KEYS.suggestion]: {
                id,
                createdAt,
                isLineBreak: true,
                type: "insert",
    ...
    normalizeNode(entry) {
      api.suggestion.withoutSuggestions(() => {
        ...
        if (node[import_platejs10.KEYS.suggestion] && // Unset suggestion when there is no suggestion id
        import_platejs10.TextApi.isText(node) && !getSuggestionKeyId(node)) {
          editor.tf.unsetNodes([import_platejs10.KEYS.suggestion, "suggestionData"], {
            at: path
          });
          return;
        }
        if (node[import_platejs10.KEYS.suggestion] && import_platejs10.TextApi.isText(node) && !getInlineSuggestionData(node)?.userId) {
          if (getInlineSuggestionData(node)?.type === "remove") {
            editor.tf.unsetNodes([import_platejs10.KEYS.suggestion, getSuggestionKeyId(node)], {
              at: path
            });
          } else {
...
// src/lib/BaseSuggestionPlugin.ts
var BaseSuggestionPlugin = (0, import_platejs11.createTSlatePlugin)({
  key: import_platejs11.KEYS.suggestion,
  node: { isLeaf: true },
  options: {
    currentUserId: "alice",
    isSuggesting: false
  ...
  ({ api, editor, getOption, setOption, type }) => ({
    dataList: (node) => {
      return Object.keys(node).filter((key) => {
        return key.startsWith(`${import_platejs11.KEYS.suggestion}_`);
      }).map((key) => node[key]);
    },
    isBlockSuggestion: (node) => import_platejs11.ElementApi.isElement(node) && "suggestion" in node,
    node: (options = {}) => {
      const { id, isText, ...rest } = options;
      const result = editor.api.node({
        match: (n) => {
          if (!n[type]) return false;
          if (isText && !import_platejs11.TextApi.isText(n)) return false;
          if (id) {
            if (import_platejs11.TextApi.isText(n)) {
              return !!n[getSuggestionKey(id)];
            }
            if (import_platejs11.ElementApi.isElement(n) && api.suggestion.isBlockSuggestion(n)) {
    ...
    nodeId: (node) => {
      if (import_platejs11.TextApi.isText(node)) {
        const keyId = getSuggestionKeyId(node);
        if (!keyId) return;
    ...
    nodes: (options = {}) => {
      const at = (0, import_platejs11.getAt)(editor, options.at) ?? [];
      return [
        ...editor.api.nodes({
          ...options,
          at,
...
// src/react/SuggestionPlugin.tsx
var SuggestionPlugin = (0, import_react.toPlatePlugin)(BaseSuggestionPlugin);
