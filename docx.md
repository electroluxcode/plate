...
扩展编辑器
使用`extendEditor`插件选项在创建后直接修改`editor`对象的属性

## 2.6 ai 插件

触发模式: 光标/选择/块选择/幽灵文本-copilot
响应模式: 聊天模式/插入模式
...
1. 所有对编辑器值的更改都应通过 [Transforms](https://docs.slatejs.org/api/transforms) 进行，以保持与选区(selection)和历史记录(history)的一致性。
